...
这条「框架 vs 产品」的轴贯穿全部 13 章。每次你看到 LangChain "什么都没做"，先想想是不是它本来就不该做。
...
读本章的第 3 节（十个最值得记住的发现），再读 [14-横向总表与选型指南](./14-横向总表与选型指南.md)。
...
架构分层决定了你未来能不能改；中间件决定了别人能不能扩展你；模型抽象层决定了你被哪家厂商绑定多深；会话持久化决定了你能不能做"回到某一步重来"这件事——而这件事比大多数人以为的重要得多。

### 如果你是要选型 / 说服团队

读 **11（安全）→ 08（MCP）→ 07（扩展分发）→ 14（选型）**
...
| [14](./14-横向总表与选型指南.md) | 横向总表与选型 | 全维度对照 + 决策树 | 选型的人 |
