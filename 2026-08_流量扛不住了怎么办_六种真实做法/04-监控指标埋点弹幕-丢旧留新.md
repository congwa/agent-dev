...
| 限制 | 具体含义 | 出处 |
|---|---|---|
| 不支持多帧消息 | SUB socket 用 CONFLATE 时不能用非空 topic 前缀（topic + payload 是两帧） | 官方文档 |
| 必须在 `bind` / `connect` 之前设置 | `zmq_setsockopt` 的 Caution 段落列了可以后设的选项白名单，`ZMQ_CONFLATE` 不在里面 | `zmq_setsockopt.adoc` |
| 只对 PULL / PUSH / SUB / PUB / DEALER 生效 | 其余 socket 类型设了不起作用 | `src/options.hpp` 的 `get_effective_conflate_option` |
