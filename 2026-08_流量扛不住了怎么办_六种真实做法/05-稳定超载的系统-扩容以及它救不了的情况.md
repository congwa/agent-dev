...
**scale-to-zero 是怎么做到的**：原生 HPA 有个死结 —— 没有 Pod 就没有指标，没有指标就永远起不来第一个 Pod。KEDA 绕开的方式是自己接管这一段：

```
if 副本数 == 0:
    # 原生 HPA 在这里瞎着，改由 KEDA 自己的 controller 轮询外部源
    队列深度 = 直接问 RabbitMQ / Kafka
    if 队列里有活:  副本 0 -> 1
else:
    交还原生 HPA，按指标正常伸缩
```

一句话：**0 到 1 由 KEDA 自己捅，1 之后还给 HPA。**
...
- **`ExpectedQueueConsumptionTime`** 的第二项（源码里是积压消息数除以消费速率）**就是 W = L / λ** —— 积压条数 ÷ 消费速率 = 清空需要多久。你可以直接写「目标：积压能在 30 秒内清空」：
...
这个公式外面还包着一层容差，写成伪代码是：

```
ratio = currentMetricValue / desiredMetricValue
if |ratio - 1.0| <= 0.1:      # 容差默认 0.1（10%）
    什么都不做                 # 比值在 1.0 附近 10% 以内不动作
else:
    desiredReplicas = ceil(currentReplicas × ratio)
```

也就是说，指标在目标值上下 10% 以内晃，HPA 是装看不见的。
...
| 仓库 | stars | 说明 |
|---|---|---|
| [aws/karpenter-provider-aws](https://github.com/aws/karpenter-provider-aws) | 7.7k | AWS 实现 |
| [kubernetes-sigs/karpenter](https://github.com/kubernetes-sigs/karpenter) | 1.9k | 云厂商无关的核心 |
...
把上面的代码和默认值合起来看，决策大致长这样：

```
dspc = ceil(60s 窗口内观测并发 / 目标值)     // 慢的那个，看长期趋势
dppc = ceil(6s  窗口内观测并发 / 目标值)     // 快的那个，专门抓突增

if 6s 窗口内的并发 >= 目标值 × 200%:
    进入 panic 模式
if 处于 panic 模式:
    只允许把副本数往上调，绝不往下调
```
...
后端变慢了（W↑），并发自然上升，副本数自动上调 —— 而 CPU 指标在这种情况下可能纹丝不动（线程都在等 IO）。

**这是比 CPU 更贴近排队论的做法**，如果你的服务是 IO 密集型，强烈建议换成并发数或队列长度当扩容指标。
...
**为什么扩容会失效？** 因为在元稳定状态下，有效产能已经被正反馈拉低到远小于标称容量。

你扩的是标称容量，但**新实例一上线就被同样的重试洪水打进同一个坏状态**（冷缓存 + 满队列 + 重试放大）。
...
Google SRE 书给了三道具体的防线：

| # | 防线 | 数值 | 书里的原话 | 出处 |
|---|---|---|---|---|
| 1 | 每请求重试预算 | 最多 3 次尝试 | Implement a per-request retry budget of up to three attempts. | [Handling Overload](https://sre.google/sre-book/handling-overload/) |
| 2 | 每客户端重试比例上限 | 10% | A request will only be retried as long as this ratio is below 10%. | [Handling Overload](https://sre.google/sre-book/handling-overload/) |
| 3 | 进程级重试预算 | 每分钟 60 次 | only allow 60 retries per minute in a process, and if the retry budget is exceeded, don't retry; just fail the request. | [Addressing Cascading Failures](https://sre.google/sre-book/addressing-cascading-failures/) |

Addressing Cascading Failures 同一章里还有一条同样重要的原则：**只在最靠近失败的一层重试，上层不再重试**。因为多层重试是**相乘**的：
...
`retryThrottling` 是一个 token bucket，规则（规范原文）写成伪代码就四行：

```
token_count = maxTokens                    // 初始 = maxTokens
每次 RPC 失败:  token_count -= 1
每次 RPC 成功:  token_count += tokenRatio
if token_count <= maxTokens / 2:  停止重试
```
...
那段代码里 0.5 到 1.5 倍之间的随机抖动系数不是可有可无的。

没有抖动的话，一批客户端会**同步**重试，形成周期性的脉冲流量 —— 你会看到一个漂亮的锯齿波，每个波峰都把服务打死一次。
...
Marc Brooker 在 [Latency Sneaks Up On You](https://brooker.co.za/blog/2021/08/05/utilization.html) 里说得很好：你优化了处理速度，延迟短暂变好了，然后业务增长会把利用率顶回原来的水平，延迟原样恶化。

**除非你同时提高了容量余量，否则优化性能不会改善延迟。**
