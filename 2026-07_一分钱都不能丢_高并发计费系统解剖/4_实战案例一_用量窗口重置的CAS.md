...
规则本身很朴素,每个请求进来先问一句"窗口过期了吗",过期就把账面重开:
...
每个请求进来:
    if 现在 >= 窗口起点 + 24h:      // 窗口过期
        已用金额 = 0               // ① 把已用金额清零
        窗口起点 = 新起点           // ② 把窗口起点换成新的
    if 已用金额 < 限额:            // 清完之后才做这个检查
        放行
```

问题就藏在缩进里:①②是两次数据库交互,而"判断过期"发生在它们之前。这道缝有多宽,事故就有多容易钻进来:
...
| 后果 | 具体表现 |
|---|---|
| 计数器失真 | 50 个并发 = 最多 50 次清零。用户在一个窗口里实际消费 $150,计数器上永远只显示最后几笔 |
| 限额白送 | **日限额 $10 形同虚设,用户变相白嫖**,并发越高薅得越多 |
| 必然发生 | 这不是偶发 bug——**每天窗口翻转的那一瞬间必然发生**:那一刻所有在途请求都会同时判定"该重置了" |
| 无声无息 | 不报错、无异常日志,只有账目慢慢对不上 |

## 4.3 加上 CAS:同一场景重放

sub2api 的解法是给 UPDATE 加一个前提——**"只有窗口起点还是刚才读到的那个旧值,才允许清零"**:
...
源码在 `internal/repository/user_subscription_repo.go`。
...
B 收到"影响 0 行"。这个 0 有歧义,两种可能含义天差地别:

| 影响 0 行的原因 | 含义 | 该怎么办 |
|---|---|---|
| 行被别人抢先改了 | 完全正常:窗口已被推进 | 无需再做任何事 |
| 行根本不存在 | 真错误:订阅没了 | 返回 `ErrNotFound` |
...
**不做这个区分的后果**:每天窗口翻转时,49/50 的请求会"报错",监控告警炸锅,而实际上什么都没坏。

CAS 输了是常态。把常态当异常处理,系统就无法运维。
...
内存.已用 = 0            // CAS 前的乐观修改
CAS 结果 = 输
// 错的做法: if 内存.已用 < 限额 → 放行        ← "假 0",打穿限额
真值 = 绕过缓存回读数据库(sub.ID)
if 真值.已用 < 限额 → 放行                      // 基于真值重新决策
```

sub2api 的做法就是这一句:CAS 之后**强制绕过缓存、直接回读数据库**。
...
这里有个读代码时容易被绊一下的地方:sub2api 判断"该不该重置"用的是**滚动 24 小时**(当前时间是否已经过了窗口起点整整一天),但重置时写入的新起点却不是"现在",而是**当天零点**——自然日意义上的今天 00:00。
