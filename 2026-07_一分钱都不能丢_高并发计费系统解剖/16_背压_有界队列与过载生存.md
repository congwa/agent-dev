...
sub2api 用 [alitto/pond](https://github.com/alitto/pond) 构造"固定 worker + 有界队列"[^1]:
...
灵魂代码在 `Submit`,骨架就是一个 `select/default`[^2]:
...
`WithTimeout(parent, d)` 的 ctx 在**两钟先到者**触发。而 sub2api 对钱的操作故意剪断这根线[^3]:
...
[^1]: `usage_record_worker_pool.go:134`。
[^2]: `usage_record_worker_pool.go:146`。
[^3]: `gateway_usage_billing.go:475`。
</content>
</invoke>
