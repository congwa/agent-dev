...
这些任务的注册点不散落在各处,全部在 `cmd/server/wire_gen.go` 的 `provideCleanup`——进程启动时统一拉起,优雅停机时统一 `Stop()` 并等 goroutine 退干净(`sync.WaitGroup` / `done channel`)。
...
PG 咨询锁(`pg_try_advisory_lock`)不碰任何表,锁 ID 由任务 key 的 FNV 哈希得来,持有期间占用一条连接,释放时 `pg_advisory_unlock` 并归还连接(`ops_advisory_lock.go:17`)。
