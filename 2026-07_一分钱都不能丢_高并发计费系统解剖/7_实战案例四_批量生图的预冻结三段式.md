...
八步里最容易被忽视的不是任何一条校验,而是**它们的先后**——②必须排在③④之前,否则一张永远过不了校验的坏单会无限重试,冻结永远解不开。

实现在 `batch_image_settlement.go:76`。
...
为什么非要拦这一下:冻结从未成功的 job 触发释放,等于**从其他用户的冻结资金池里凭空造出余额**。守卫实现在 `usage_billing_repo.go:341-346`。
...
派生逻辑在 `batch_image_billing_hold.go:18-27`。三个单号复用主流程那张 `usage_billing_dedup` 表,第 6 章 6.5 的机制原样复用,没有新造轮子。
...
Redis 锁会丢(过期、脑裂)。两个 worker 同时推进同一 job 时,DB 层还有一道**短事务悲观锁**(`batch_image_repo.go:357-384`):
