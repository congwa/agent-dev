...
每条结论都能从前面某一节的画面里重新推出来:

- **读-改-写三步分离必然丢更新**——因为 1.1 里那两处没写出来的缝隙,读和写之间别人能插进来,第三步写回去的是"整体覆盖",不是"+50"这个动作;
- **先检查再动手一样会丢**——因为 1.3 证明它和 1.1 是同一张缝隙图,只是过期的东西从一个数字换成了一个判断结论,而判断结论过期得更悄无声息;
- **"缝隙才几微秒,不用管"是错觉**——因为 1.4 的算术摆在那儿:概率 × 请求量 = 必然,大流量下低概率事件就是日常事件;
- **这类 bug 必须在写代码时就防**——因为 1.2 那张危险点清单说得明白:它测不出、不报错、上量才炸,丢的往往还是钱,等你从对账单反推回来,已经过去一个月了;
- **解法方向只有一个**——因为病根从头到尾只有一个:基于过期的认知做写入,药方就是把读-改-写焊成一个不可分割的整体。

具体怎么焊——悲观锁、CAS、原子增量三条路线各自的成本和场景,下一章逐条拆开讲。
