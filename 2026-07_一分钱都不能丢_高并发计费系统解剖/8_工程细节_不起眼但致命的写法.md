...
**不用的后果**:扣完再 SELECT 一次去拿新余额。多一次往返是小事,大事是 **SELECT 读到的可能已被其他并发又扣过**——拿着一个"不是这笔扣完"的值去做阈值判断、发低额通知,全是错位的。
...
**不这样写的后果**:先 SELECT 查存在性再 INSERT = TOCTOU。并发下两个执行流都"查到不存在"然后都插,结果要么一个吃唯一约束报错(若没设计这个分支,就会被当成系统错误),要么都以为自己是第一个。
...
**不这样写的后果**:在每个 error 分支手写 Rollback——总会漏一个分支,尤其是 panic 路径。漏掉的那个事务挂着不提交也不回滚,**占着连接和锁**直到超时。
...
singleflight 里 10 个请求共享 1 次数据库回源,回源函数要用 `context.Background()+超时`,不能用第一个请求的 ctx。

**不做的后果**:第一个请求恰好断线,其余 9 个陪着失败。
...
**规矩:handler 在提交任务前把值取出来,当参数传进任务。** 不做的后果:工人取到零值,静默走错分支。
