...
回顾第 6 章的扣费幂等:每笔计费动作先往去重表里抢一个凭证,抢到才允许扣费。
...
关键在于,幂等凭证的价值是有保质期的:它防的是**重试窗口内**的重复请求。

一笔三天前的计费,不可能再有合法重试到达——它的凭证已经是纯粹的垃圾,却还在给每一次新抢占垫高成本。
...
上一节已经说完后果。这个方案最大的迷惑性在于**短期内毫无症状**。

前三个月一切正常,第六个月扣费 P99 缓慢爬升,没有任何一次变更能对上时间线——增长型的问题从不在代码评审里报警。
...
分开客户端、服务、幂等表三个角色看,穿透发生在哪一步一目了然:
...
正确的保质期必须从重试行为倒推:**凭证的 `expires_at` ≥ 所有合法重试可能到达的最晚时刻**,包括异步管道里的排队重试、进程重启后的恢复重试。

sub2api 的解法是把这个决定权推给写入方:

```
写入方(业务):  凭证.expires_at = 按本业务的最晚合法重试时刻算出来的时间
清理任务(基建): DELETE ... WHERE expires_at < now()   // 不猜业务,只看这一个字段
```

| 谁 | 负责什么 |
|---|---|
| 写入方 | 按业务语义决定每条记录的 `expires_at` |
| 清理任务 | 只认 `expires_at < now()` |

过期时间直接存在每条记录上,**"活多久"是业务决策,"谁收尸"是基建职责**,两者解耦。
...
一句话:每分钟一轮、每轮最多 500 条、启动时先跑一轮、失败只打日志、多实例不选主。

`idempotency_cleanup_service.go` 的骨架很朴素:一个 60 秒一跳的定时器,外加一个"启动先跑一轮"的动作。每次触发只做一件事——按过期时间删,一次最多 500 条,出错就记日志,交给下一轮。完整实现照抄文末[附录](#附录可以照抄的模板)。整个循环画出来是这样:
...
`idempotency_cleanup_service.go` 的清理循环全文:
