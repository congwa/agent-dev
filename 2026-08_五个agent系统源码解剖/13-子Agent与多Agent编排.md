...
> 基于 Pi coding-agent、Codex、LangChain v1（另有第四、第五样本 Grok Build、DeepSeek Harness）源码核对，出处收在文末脚注，可照抄代码收在附录。

子 agent 的第一动机不是「并行」，是「隔离上下文窗口」：让别人去读 50 个文件，自己只把几百字的结论带回来。三家给出的答案完全不同：Codex 内置了两代、总共十一个工具，让模型自己判断何时该委派；Pi 明确拒绝内置这套东西，只在示例里给了一个起 `pi` 子进程的扩展；LangChain v1 干脆没有 `spawn_agent` 这个概念，因为在 LangGraph 里，「一个 agent 是另一个 agent 的工具」本来就只是一次函数调用。
...
但现在你的对话历史里已经躺着 20 个文件的完整内容，大概 8 万 token。接下来你说「好，现在帮我给登录加个二次验证」，这 8 万 token 会跟着每一次 LLM 调用一起发出去，一直发到你关掉这个会话为止。
...
先看立场本身。README 里原话是「No sub-agents. There's many ways to do this. Spawn pi instances via tmux, or build your own with extensions, or install a package that does it your way.」[^1]

同一节里紧邻着一条逻辑完全一致的表述：「No background bash. Use tmux. Full observability, direct interaction.」[^2]这两条要连起来读才能理解 Pi 的论证。它的主张是：一个跑在后台、你看不见其内部的执行单元，是可观测性的净损失；而 tmux 已经解决了这个问题——开一个 pane 跑第二个 pi 实例，你能实时看到它每一步在做什么，能随时切进去直接跟它对话，能在它跑偏的时候立刻纠正。
...
子进程真实的起法逐句对得上这段伪代码[^3]。四个固定参数各有讲究：
...
系统提示词的传递方式很朴素：写到临时文件，再用 `--append-system-prompt` 指过去[^4]，而且是追加，不是替换。子 agent 仍然带着 pi 的基础系统提示词，agent 定义只是叠加人格。

中断传播的真实实现同样对得上伪代码那两行[^5]：工具执行拿到的 `AbortSignal`（Pi 工具系统的标准参数）被直接转成 SIGTERM，5 秒宽限期后 SIGKILL。用户 Ctrl+C 能穿透到子进程，因为进程边界清晰，这件事在 Pi 这里几乎是免费的。

回传上限由四个常量钉死[^6]：`MAX_PARALLEL_TASKS = 8`、`MAX_CONCURRENCY = 4`、`COLLAPSED_ITEM_COUNT = 10`、`PER_TASK_OUTPUT_CAP = 50 * 1024`（50 KB）。这个值比 Codex 的 1000 token 宽松一个数量级，Pi 的示例扩展更倾向于「全须全尾地回传」，隔离效果因此打了折扣。

README 里还专门区分了「模型可见的输出」和「tool details 里的完整结果」：截断只发生在前者，UI 展开还能看到全部。这是一个值得抄的分层。

agent 定义本身就是很好的 prompt 设计素材。`scout` 的关键在于它明确告诉模型「你的读者没见过你读过的文件」，完整定义照抄附录 A[^7]：输出格式要求列出精确的文件行号范围，再指明该从哪个文件开始看。「返回精确行号范围」这条要求解决的正是 1.1 的带宽悖论：不回传文件内容（省 token），但回传足够精确的坐标，父 agent 如果真的需要可以定点读取，不必重新探索。这是一个非常划算的折中。

`reviewer.md` 里则藏着一句对工具隔离的诚实评价[^8]：「Bash is for read-only commands only… Assume tool permissions are not perfectly enforceable; keep all bash usage strictly read-only.」`--tools read,grep,find,ls,bash` 能把工具集限制成只读工具加 bash，而 bash 本身是万能的。作者知道这个洞补不上，于是在提示词里再叮嘱一遍。这句话应该被每一个「用工具白名单实现沙箱」的人读一遍。

工作流被表达为 prompt 模板而不是代码，`implement.md` 是完整例子，同样照抄附录 A[^9]：扩展只提供了 `chain` 这个原语，「scout → planner → worker」这条流水线靠一段自然语言让主模型去组装 `chain` 参数。这里有一个容易忽略的架构选择：编排逻辑不在代码里，在提示词里。好处是改流程不用改代码，坏处是流程的执行没有任何保证，模型完全可以决定跳过 planner。
...
`models.json` 里，`gpt-5.6-sol` 和 `gpt-5.6-terra` 有一个别的模型没有的推理档位，描述原文直接写着「Maximum reasoning with automatic task delegation」[^10]。这不只是文案，挑选委派策略的判定顺序是这样的：
...
也就是说推理档位直接决定委派策略，但配置里给了 hint 文本时轮不到档位说话；真实实现逐句对得上这段伪代码[^11]。

这个模式最终变成一段注入到上下文里的 developer 消息。「Explicit」那份原文开头是「之前允许主动委派的指令作废，除非用户或 AGENTS.md/skill 明确要求，否则不要派子 agent」；「Proactive」那份反过来，开头是「之前要求用户明确要求才能派子 agent 的指令作废，并行工作能明显提速或提质时就用子 agent」[^12]。两段文本都以「之前那条相反的指令作废」开头，因为这段 developer 消息是随会话状态增量注入的（`MultiAgentModeState` 实现了 `render_diff`），历史里可能残留着上一次的相反指令，必须显式覆盖。这是一个在「上下文是 append-only 日志」这个约束下做状态机的经典技巧。
...
内置角色表定义了当前启用的三个角色：`default`、`explorer`、`worker`；`awaiter` 被整段注释掉了（注释写着「Awaiter is temp removed」）[^13]。

`explorer` 的描述原文是：「Use `explorer` for specific codebase questions. Explorers are fast and authoritative. … Typically, you should trust the explorer results without additional verification. You are still allowed to inspect the code yourself to gain the needed context! … You are encouraged to spawn up multiple explorers in parallel when you have multiple distinct questions to ask about the codebase that can be answered independently. … Reuse existing explorers for related questions.」[^13]「Typically, you should trust the explorer results without additional verification」这句是 1.1 带宽悖论的正面应对：既然回传的是压缩结论，就必须显式告诉父 agent 不要复查，否则隔离带来的节省会被复查吃光。

有意思的是，`explorer` 角色对应的配置文件是 0 字节的空文件。也就是说 explorer 角色不覆盖任何配置，它和 default 的唯一区别就是那段描述文字。角色系统的真实内容是提示词，配置文件只是预留的槽位。

被注释掉的 `awaiter` 角色反而是有内容的，展示了这个槽位本来该怎么用，完整配置照抄附录 B[^14]：`reasoning_effort` 调到最低档，超时上限拉到一小时，唯一职责是盯着一个长时间运行的命令，规则逐条禁止它修改任务、解读任务、擅自停止等待。这是一个几乎不消耗智力的角色，它的价值不是「思考」而是「代替父 agent 承担等待」：父 agent 不用把一小时的构建日志读进自己的上下文，只需要在结束时收一句「成功/失败」。这是子 agent 作为上下文管理手段最纯粹的形态，跟并行毫无关系。

而 `worker` 角色的描述，正面回答了 1.5 的并行写冲突：要求「显式分配任务的所有权（文件/职责）……这有助于避免合并冲突并明确责任」，还要求「始终告诉 worker 它们在代码库里不是孤身一人，不应该回滚别人的改动，而应该调整自己的实现去适配别人的改动」[^15]。

请注意这里的机制层面事实：委派时运行时会显式把子 agent 的工作目录钉成父 turn 的工作目录[^16]。所有子 agent 共享父 agent 的工作目录，没有 worktree 隔离，没有容器隔离。三个 worker 并行改同一个仓库，冲突的唯一防线就是上面那段提示词里的「ownership」约定。

v1 的工具描述里还追加了一条更操作性的要求：「For code-edit subtasks, decompose work so each delegated task has a disjoint write set」，把写集不相交的责任推给了父 agent 的规划能力。这是一个诚实的设计：Codex 没有假装解决了这个问题，但也必须说清楚，这是纪律不是机制，模型不听话时没有任何东西会拦住它。
...
内置了一份 100 个名字的列表：Euclid、Archimedes、Ptolemy、Hypatia、……、Turing、Feynman、Lovelace、McClintock、……、Socrates、Confucius、Mencius、……最后一个是 `Jason`（大概是个彩蛋）[^17]。

名字用完之后的处理是一段序数词生成逻辑，还处理了 11/12/13 的英文例外，完整实现照抄附录 C[^18]：一百个名字用完之后重置，第二轮叫「Newton the 2nd」。为了一个内部标识符写序数词后缀的英文规则，这是很明显的 UX 投入。为什么值得？因为不透明的会话 id 用户没法在对话里指代。有了昵称，用户可以说「让 Newton 停下」「Curie 那个结果不对」。

这些昵称还会被注入到 agent 自己的环境上下文里，格式很简单：有昵称就写成「- 引用: 昵称」，没有就只写「- 引用」[^19]。

#### 回传：1000 token 的硬墙

子 agent 完成时，它的最终消息被包装成一条 assistant 角色的上下文片段注入父 agent，格式固定为「Message Type: FINAL_ANSWER / Task name: … / Sender: … / Payload: …」四段[^20]。

而错误路径上有一个硬性的 token 预算：`COMPLETION_MESSAGE_MAX_TOKENS = 1_000`，`COMPLETION_MESSAGE_ENVELOPE_TOKEN_RESERVE = 100`，二者相减得到 `ERROR_MAX_TOKENS`；失败时返回给模型的固定文案是「This agent's turn failed. If you still need this agent, use the available collaboration tools to give it another task.」[^21]失败的处理值得学：错误不是异常，是一段给模型看的自然语言，而且附带了「你接下来可以做什么」的行动建议。这比抛一个 `SubagentError` 有用得多，LLM 拿到这句提示就知道下一步该 `followup_task` 而不是放弃。

⚠️ 未确认：`COMPLETION_MESSAGE_MAX_TOKENS` 这个常量在代码里只被用于计算 `ERROR_MAX_TOKENS`，成功路径上的 `AgentStatus::Completed(Some(message))` 直接原样 clone，没看到截断。所以「1000 token 硬墙」目前只能确认适用于错误消息；成功消息是否在别处被截断我没找到证据。
...
| 闸门 | 值 | 超限时 |
|---|---|---|
| 深度上限 | 默认 1（`DEFAULT_AGENT_MAX_DEPTH`[^22]），即默认不允许子 agent 再派子 agent | 返回一句人话「Agent depth limit reached. Solve the task yourself.」[^23] |
| 并发上限 | `agents.max_concurrent_threads_per_session` | 返回 `AgentLimitReached` |
...
| `SubagentStart` / `SubagentStop` hook | 外部可编程的钩子[^24]，payload 带子 agent 身份与独立转录路径 |

`agent-graph-store` 这个 crate 的 doc comment 写得很到位：「Storage-neutral parent/child topology for thread-spawned agents.」

hook 输入 payload 的完整字段照抄附录 D[^25]：`agent_transcript_path` 和 `transcript_path` 是两个不同的字段，这就是「上下文隔离」在存储层的体现，子 agent 有自己独立的一份转录。想做审计的话，这个字段是唯一能让你看到子 agent 内部到底发生了什么的入口。
...
框架里没有任何叫 subagent 的运行时机制。官方测试用例就是最权威的用法示范[^26]：定义一个具名的 `weather_agent`，在一个普通 `@tool` 函数 `call_weather` 里直接调用 `weather_agent.invoke({"messages": [...]})`，取回结果里最后一条消息的文本作为返回值，再把这个工具挂给一个叫 `supervisor` 的 agent。完整代码七行，见附录 E。

这七行代码就是 LangChain v1 的完整子 agent 方案，值得逐点拆：

- 上下文隔离是天然的。往子 agent 传的是一个全新的 messages 列表，子 agent 的 state 和父 agent 的 state 是两个 Python 对象，没有任何共享。隔离不需要机制，因为默认就不共享，要共享才需要写代码。
- 回传形态完全由你决定。这里返回的是最后一条消息的文本，但你可以返回摘要、返回结构化 dict（用 `response_format`）、返回文件路径，返回什么都行。这是三家里唯一把「回传什么」完全交给开发者的。1.1 的带宽悖论在这里不是框架问题，是你的问题。
- 代价同样明显：`invoke()` 是同步阻塞的。父 agent 的这个工具调用会卡到子 agent 跑完。要并行你得自己用 `asyncio.gather` 或者 LangGraph 的 fan-out（见下）。没有 `wait_agent`，没有邮箱，没有中途通信，除非你自己实现。

#### `name=` → `lc_agent_name` → 可观测性

`create_agent(name=...)` 做的事情很小：往 `RunnableConfig` 的 `metadata` 字段里塞一个 `lc_agent_name`[^27]。但这个名字是整条可观测性链路的锚点。编译时注册的 transformer 列表里，`SubagentTransformer` 紧跟在 `ToolCallTransformer` 后面[^28]。

`SubagentTransformer` 的职责，模块 docstring 写得很清楚[^29]：「Surface nested named agents as typed `run.subagents` handles. A subagent boundary is a nested run whose `lc_agent_name` is set and differs from its parent namespace's `lc_agent_name`. Plain subgraphs inherit the parent's name (so they compare equal and are excluded); unnamed agents have `lc_agent_name == None` (also excluded). For genuine subagents the base also recovers the originating tool call and exposes it as a `cause`……」

这里要纠正一个容易产生的误解：这个文件不是「把 agent 变成工具」的实现，它是一个流转换器。它的作用是：当嵌套的 agent 在跑的时候，把属于它的那部分事件从主事件流里劈出来，包成一个独立的 `SubagentRunStream` 句柄推到 `run.subagents` 上。也就是说，1.4 说的「黑盒里的黑盒」，LangChain 的答案是把黑盒开个窗：跑 `stream_events`，遍历 `run.subagents` 拿到的每个 handle，把嵌套的事件流耗尽就能拿到完整信息；测试断言里那个 handle 带着 `name == "weather_agent"` 和 `cause == {"type": "toolCall", "tool_call_id": "call_w"}`[^26]。

`handle.cause` 是一条因果边：这个子 agent 是被哪个 tool_call 触发的。有了它，UI 就能把「主 agent 调了 call_weather」和「weather_agent 内部的 12 个事件」在时间轴上正确地嵌套起来。

这在三家里是最完整的子 agent 可观测性模型，Codex 发的是扁平的 turn item（靠 `agent_path` 自己拼层级），Pi 的示例扩展是逐行解析子进程 stdout。

判定边界的代码也把 trade-off 说清楚了，注释原文是[^30]：「Surface any nested run carrying an `lc_agent_name`……A same-named nested agent — e.g. a subagent that invokes itself — re-asserts its own name and is surfaced. Unnamed runs (None) are excluded. Trade-off: a non-agent subgraph that inherited the parent's name also surfaces; null `lc_agent_name` when invoking such a graph to exclude it.」「有名字的就算子 agent」是一条启发式规则，注释里主动承认了它的假阳性场景和绕过方法。这种诚实度在框架代码里不常见。
...
动态 fan-out（map-reduce）用 `Send`。`Send` 类的 docstring 直接给出了它的定位[^31]：「A message or packet to send to a specific node in the graph. The `Send` class is used within a `StateGraph`'s conditional edges to dynamically invoke a node with a custom state at the next step. … One such example is a "map-reduce" workflow where your graph invokes the same node multiple times in parallel with different states, before aggregating the results back into the main graph's state.」

文档里给的例子直接就是 fan-out 的骨架：一个 `continue_to_jokes` 函数遍历 `state["subjects"]`，为每个 subject 生成一个 `Send("generate_joke", {"subject": s})`，挂在起始节点的条件边上。这解决了 Codex 和 Pi 都需要模型自己拿捏的事：并行度由数据决定，不由模型决定。有 17 个文件要审查就派 17 个，代码里写死，模型不参与，也就不会「忘了派」或者「派太多」。
...
状态隔离用 subgraph 的独立 schema。`StateGraph` 从构造时就区分 `input_schema` 和 `output_schema` 两套类型，`add_node` 也接受 per-node 的 `input_schema`[^32]。这意味着：一个子图可以有自己完全不同的 state 定义，父图只通过 `input_schema` 定义的那几个字段喂进去、只通过 `output_schema` 定义的那几个字段收回来。中间产生的所有中间状态在父图里根本不存在。

这是「上下文隔离」在图模型里的表达，而且它比另外两家更精确：Codex 和 Pi 的隔离粒度是「整个上下文」（要么全 fork 要么全不 fork），LangGraph 的隔离粒度是字段级，由类型定义。

#### 现成的编排模式在独立包里

仓库里搜不到 supervisor / swarm 的实现，`prebuilt` 目录只有 `chat_agent_executor.py`、`tool_node.py`、`interrupt.py` 等。它们在独立发布的包里：
...
另外，`create_deep_agent`（`deepagents` 包，在 langgraph 仓库里只作为集成测试的依赖出现）提供了最接近 Codex 形态的东西：一个 `task(subagent_type, description)` 工具。集成测试的模块 docstring 这样描述它[^33]：「`create_deep_agent` builds a graph whose `task` tool dispatches to one of its configured `SubAgent`s. When the supervisor's model issues a `task(subagent_type="researcher", description=...)` tool call, the sub-agent runs as a nested invocation and the v3 streaming server emits the subagent's lifecycle, messages, and tool events under a scoped namespace.」

`langgraph-supervisor` 那条弃用提示值得单独想一下。官方的判断是：把编排封装成一个高阶 API（`create_supervisor(agents=[...])`）反而降低了控制力，因为多 agent 系统的难点从来不是「怎么调度」，而是「每个 agent 看到什么上下文」。原话里的 "more control over context engineering" 就是这个意思。这跟本章第 0 节的判断是一致的：子 agent 的本质是上下文管理，编排只是它的外壳。
...
一句话：Grok Build 把子 agent 做成一等公民，形态是「后台任务池」而非 Codex v2 的 actor 网络；它是四个样本里唯一给 1.5 节的并行写冲突提供机制层解法的（`isolation: "worktree"`），也是唯一把「异步」设为默认的。

机制：入口是 `task` 工具，每个子 agent 是一个独立的 ACP 子会话。`TaskToolInput` 的字段本身就是设计立场的清单[^34]：

| 字段 | 值/默认 | 对照 |
|---|---|---|
| `run_in_background` | 默认 true，工具描述原话「This is set to true by default」[^35] | 三个主样本默认全是同步等待（Pi 等子进程退出、Codex v1 `wait_agent`、LangChain `.invoke()`）；Grok 直接把 1.2 节的邮箱模型设为默认 |
| `capability_mode` | read-only / read-write / execute / all | 声明式能力面，对照 Pi 的 `--tools` 白名单 |
| `isolation` | none（默认）/ worktree | 工具描述逐字：「Worktree mode prevents the child's edits from…」[^36]，用的是独立 git worktree，完成后保留并返回路径。这就是 6 节经验第 4 条里「三家都没做」的那件事 |
| `resume_from` | 已完成子 agent 的 id | 继承 peer 的 transcript 和工具状态，与 worktree 互斥[^37] |
...
内置三型 `general-purpose` / `explore`（只读）/ `plan`（只读架构师）[^38]，与 Codex 的 default/explorer/worker 三角色几乎一一对应。

用户自定义 agent 是 `.md` + frontmatter，发现顺序里原生认用户级配置目录[^39]，Pi 用同样的 markdown+frontmatter 格式但只认自己的目录，Grok 直接兼容 Claude Code 的。

嵌套深度默认 1[^40]，与 Codex 的默认值相同，两家产品在 1.6 节的递归爆炸问题上独立收敛到同一个数。

上下文继承是第三种答案。第 3.2 节讲过 Codex v1（默认不继承）到 v2（默认全继承）的摇摆，Grok 的 `InitialContextSource` 给了三档[^41]：`New`（默认，只带 task prompt + 压缩版 AGENTS.md）、`Forked`（父会话史规范化成一条 `<background_context>` 用户消息，最多 3 个完整 turn 逐字保留、更早的只留统计摘要，并剥掉 system-reminder/git_status 等噪声标签[^42]）、`Resumed`。
...
回传与管理：tool result 带 `SubagentCompletedOutput`，output 加统计信息加 resume 提示[^43]；后台模式先返回 id，由 `get_task_output`（可多 id 聚合等待，上限 20）/ `wait_tasks`（wait_any|wait_all）/ `kill_task` 管理[^44]。

没有 agent 互发消息：父模型对运行中的子 agent 只有 poll/wait/kill，`resume_from` 仅限已完成者。也就是说 Grok 停在了 Codex v1.5 的位置：邮箱化了等待，但没有走到 v2 的任意 agent 网络。防提前收尾靠 `Stop` hook 的 payload 枚举在飞的后台任务[^45]。

编排的第四种载体。本章看过三种编排逻辑的居所：Pi 写在 prompt 模板里、Codex 放在模型脑子里（ultra 档自主委派）、LangChain 写成 Python 代码。

Grok 的 `workflow` 工具给了第四种：模型现场生成 Rhai 脚本，host 函数只有 `agent()`/`parallel()`/`phase()`/`log()`[^46]，预算护栏是 `DEFAULT_AGENT_BUDGET=128`、上限 1024、`MAX_HOST_CALLS=10_000`，支持 `validate_only` 干跑和 `resume_from_run_id` 断点续跑。
...
一句话：前四个样本各自选了一种隔离机制（Pi 子进程、Codex 同进程 thread、LangChain Python 对象、Grok ACP 子会话），dsh 把这个选择本身做成了 seam：`ctx.subagents` 是具名 provider 注册表，6 个实现并存，其中两个直接把一次委派交给真的 `claude` CLI 和真的 `codex app-server`；同时它给出了本章前面没有的第五种「能力隔离」答案：子 agent 的工具集不是白名单，而是它加入的那棵插件树。
...
代码里确实有 6 个包各注册一个 provider[^48]：

| provider | 隔离机制 | `capabilities` | `inheritsParentContext` | `prepareContinuable` |
|---|---|---|---|---|
| `spawn` | 同进程新 Agent，不带任何 seed | 四项全 true | false | 有 |
| `fork` | 同进程新 Agent + 父日志 completed-turn 前缀作 seed | 四项全 true | true | 有（但默认配置不用） |
| `acp` | 外部 ACP 子进程 | 全 false | false | 无 |
| `codex` | 真的 `codex app-server --stdio` 进程 | `NO_START_CAPABILITIES` | false | 无 |
| `claude-code` | 真的 Claude Code CLI，走官方 Agent SDK | `NO_START_CAPABILITIES` | false | 无 |
| `dsh-sdk` | 另一个 dsh 进程，走 TS SDK | `NO_START_CAPABILITIES` | false | 无 |

`spawn`/`fork`/`acp`/`codex`/`claude-code`/`dsh-sdk` 六行的具体坐标见脚注[^49]。六个 provider 按隔离机制分成两类：
...
`NO_START_CAPABILITIES` 的注释把代价写得很直白[^50]：跨进程的孩子没法兑现 `outputSchema`/`maxDepth`/`toolFilter`/`persona`，所以服务在 `start()` 之前就用 `SubagentError('UNSUPPORTED_CAPABILITY')` 拒掉[^51]，用注释原话说是「never accepted-then-ignored」。

这是本章第一次看到有人把「工具白名单在跨进程场景下不可执行」这件事做成了类型化的能力协商，而不是像 Pi 的 `reviewer.md` 那样在提示词里叮嘱一句。

选哪个 provider 不是工具参数，而是加载几个工具实例。默认组合[^52]挂了两个 `tool-subagent`：`subagent`（provider=spawn，`backgroundMode: continuable`）和 `subagent_fork`（provider=fork，`one-shot`）。示例组合里还能再挂 `subagent_codex` / `subagent_claude_code`[^53]，两者都被迫写 `maxDepth: provider-managed`，递归预算归对方运行时管。
...
而且 `inheritsParentContext` 这个布尔值的主要用途是生成不同的工具描述[^54]：fork 那份写「It already sees this conversation's completed turns… state only what is new」，spawn 那份写「it does not see this conversation」。注释解释了为什么必须这样：给 fork 用 spawn 的措辞是说谎。它还有第二处用途：`tool-ralph` 拿它做准入校验，直接拒掉继承型 provider[^55]。

fork 的 seed 口径与 Grok 的衰减式继承不同，它是无损的 completed-turn 前缀（止于最后一个 `turn/end`，不含在飞的那个 turn），且只在创建时抓一次[^56]。没有 Grok 那种「近 3 turn 逐字、更早的摘要」的压缩。

### 9.3 第五种答案：子 agent 的能力集是一棵插件树

这是本章此前没有出现过的形态。dsh 的模型可见工具几乎全部挂在 agent preset（随仓库发了四个预设：code/standard/cordis/minimal）上，而子 agent 的组合窗口做三件事，完整代码照抄附录 F[^57]：先从父 ctx 继承 `agentPresets` 的组合结果，再在系统提示词里插入一段固定的委派上下文（顺序号 120，卡在沙箱策略 110 和审批策略 115 两句之后），如果配置了 persona 就再插一段人格提示词，最后如果配置了 `toolFilter` 就在继承来的工具树上做一次交集裁剪。

所以 `toolFilter` 只是在继承来的那棵树上再做交集，不是能力的来源。

同一段函数的注释点明了这个设计的脆弱处：一个没 join preset 的孩子会看到空的工具注册表，正因为如此，join 和 per-child 注册被强行写在同一个函数里，原话是「Taking the parent as a parameter is what makes that omission unrepresentable at the call sites」。preset id 还会写进子会话 header[^58]，理由是冷读转录时必须用同一套工具集重建历史。

比工具白名单更硬的一条：委派时强制把 approval 策略钉成 `'never'`，不管父 agent 自己是什么策略[^59]，并作为 `source: 'delegation'` 事件落进子日志。配套的固定上下文句子告诉子 agent[^60]：
...
对照第 2.3 节 Pi 作者那句「Assume tool permissions are not perfectly enforceable」，dsh 在审批面上把这个洞堵成了机制（子 agent 无法通过弹窗提权），在 bash 万能这一面仍然没堵。

### 9.4 continuable 子 agent：比 Codex v2 更彻底的 actor 化，外加一条子→父推送

默认的 `subagent` 工具是 `backgroundMode: continuable`，`run_in_background` 默认 true[^61]，与 Grok 一致，与三个主样本相反。

返回的不是 job id 而是持久的 child session id（`subagentId: started.childId`）[^61]，子 agent 是一个「至多一个活 Activation 的持久 Session」，三态 `running` / `waiting` / `settled` 由 Agent 静默性和 owned-child 集合推导，不维护第二个状态机[^62]。

`send_message` 冷 resume 已落盘的孩子，`interrupt_agent` 能打断任意后代而不只是直接子女，工具描述原话是「The target may be your direct child or a deeper agent created under you」[^63]，且只停当前 turn、排队消息原地保留。
...
真正的新东西是 `report`：一个只注册进 continuable 子 agent 自己 scope 的工具[^64]，让孩子在不结束自己 turn 的情况下主动把结论推给直接父亲，`reportDelivery` 可选 `wakeup`（默认，唤醒父亲开一个新 turn）或 `quiet`（只入上下文不唤醒）。

它的提示词直指第 1.1 节的带宽悖论，而且给的是和 Pi scout「回传坐标」不同的解法[^64]：
...
另外，Activation 结算时 runtime 还会无条件给父亲发一条 `subagent-settled` 通知，和孩子自己写的 `report` 用不同的 `kind`，理由写在类型注释里：合并两者等于「credit the child with words it never wrote」[^65]。这个「运行时的话」和「孩子的话」分开记账的做法，四个既有样本都没有。

### 9.5 编排：一个引擎，两个消费者（模型写脚本 / 部署写脚本）

`ctx.workflowEngine` 与 Grok 的 `workflow` 工具形态几乎一样，但脚本语言是普通 JavaScript（`node:worker_threads` 里一个 `vm` context），host 全局只有 5 个函数加一个数据全局[^66]：`agent` / `parallel` / `pipeline` / `phase` / `log` / `args`。

比 Grok 的 Rhai 多一个 `pipeline(items, ...stages)`，逐项流水线、阶段之间没有 barrier，工具描述明确说「prefer this for multi-stage work」[^67]。

真正拉开差距的是 `agent(prompt, opts)` 的 `opts.schema`：传 JSON Schema 就返回校验过的结构化对象，脚本可以直接在 JS 里对它做计算，支持的选项是 `label`/`phase`/`schema`/`provider`/`model`[^68]。第 8 节记录的 Grok host 函数清单里没提到这一项（该节没有展开 Rhai `agent()` 的可选参数，未在 Grok 仓库复核）。

而紧接着的一个常量同样值钱：`effort`/`isolation`/`agentType` 三项被明确标成「Deferred Claude Code options we name explicitly in the rejection message」[^69]。`isolation` 被逐字列为拒绝项，测试断言脚本会因此整个死掉（断言检查报错信息包含「"isolation" is deferred」）[^70]。也就是说 dsh 知道 Grok 的 worktree 隔离长什么样，并且明确没做。

护栏：`maxConcurrentAgents` 默认 0，自动算成 `min(16, max(1, cores-2))`[^71]、`maxTotalAgents` 1000（注释原话「a runaway-loop backstop」）、单次 `parallel/pipeline` 最多 4096 项、同步片 5s、取消宽限 5s[^72]。

第 8 节批评 Grok 时说「脚本本身也是模型生成物，跳过 planner 的问题只是从 prompt 层挪到了 DSL 层」。dsh 给了这个批评一个正面回应：同一个引擎上还挂着 `tool-ralph`，脚本是编译期写死的常量 `RALPH_SCRIPT`。循环体长这样：
...
模型能碰的只有 `objective` 和 `maxRounds`，注释直说「The model supplies data only; it cannot alter the loop, provider route, schema, or handoff validation.」[^73]
...
- 并行写冲突完全不管。子 agent 共享父会话 cwd[^74]，无 worktree、无锁，`isolation` 是拒绝项。连 Codex `worker` 角色那种「明确分配文件所有权」的提示词约定都没有：全仓搜索 worktree 相关字样，命中全部落在 git 工具链与开发/翻译文档、Windows ACL 注释和上面那条拒绝测试上，子 agent 与 workflow 引擎的源码里一处都没有。第 6 节经验第 4 条「Grok 做了、三家没做」在五样本尺度下变成 1:4。out-of-process provider 可以配一个固定 cwd[^75]，但那是部署级常量，不是每个孩子一份。
- 递归上限默认 3，不是 1[^76]。Codex 和 Grok 独立收敛到 1，dsh 是唯一放到 3 的，正好落在第 7 节「递归深度 > 2 就该退回单 agent」的红线之外。深度是持久的、单调的：会话头里的委派深度与运行时的子 agent 深度取大[^77]，冷 resume 无法把自己降回顶层。
- 直接委派没有并发上限。`maxConcurrentAgents` 只存在于 workflow 引擎；`ctx.subagents` 侧只有一句「shared capacity controller may delay」的接口注释[^78]，全文搜索并发/容量/信号量相关字样，除这句注释外只剩 listing 的冷读并发常量（`COLD_READ_CONCURRENCY = 4`）[^79]和几处并发安全说明，没有任何准入闸门。模型连着发 20 个后台 `subagent` 调用，没有东西拦。
- `schedule` 不是多 agent 设施，它是会话内的定时提醒（after/at/every，其中 `every` 最小 5 分钟），提醒以普通 follow-up turn 回到原会话，不起新 agent。后台线走的是通用 job 注册表：`job_output` / `job_list` / `job_kill`[^80]加完成通知注入 owner 的下一步或唤醒空闲 owner，一次性子 agent 的后台模式就复用它[^81]。

### 9.7 可观测性：子 agent 是可以走进去说话的会话

第 1.4 节说子 agent 是「黑盒里的黑盒」，Pi 的立场是宁可用 tmux 换可观测性。

dsh 的答案比另外四家都远：子 agent 的转录就是一个普通 Session，Web 端在父会话头部挂一棵可懒加载展开的目录树，点任意深度切进去[^82]。一次性子 agent 打开是只读执行记录；continuable 子 agent 只要父亲还活着，输入框就是可用的，打的字直接进它的 FIFO inbox，Stop 走中断工具。

也就是说 Pi 用 tmux 换来的「实时看到 + 随时切进去直接对话」，dsh 在保留程序化编排的前提下拿到了。代价写在同一份 README 的 Known Limitations 里：目录树没有持久的结果状态，只有 running/inactive，分不清完成、失败还是取消。

模型侧则相反地保守：`report` 是唯一的中途回传，工具描述逐字承诺「You receive its result, not its intermediate steps.」（fork 那份；spawn 那份是「The subagent returns its result, not its intermediate steps.」）[^83]

人看得见全部，模型只看得见结论，这个分层比 Pi 的「模型可见截断 50 KB / tool details 看全部」更彻底。

### 9.8 本节未确认

- ⚠️ 一次性（one-shot）子 agent 最终输出有没有 token/字符上限没找到。`SubagentResult.output` 是「最后一条非空 assistant 消息」原样带回[^84]，`tool-subagent` 只在失败路径拼一句「Partial output before the run ended:」，没看到截断。对照 Codex 的 1000 token 硬墙（该数值本身在第 7 节存疑 1 里被标为未确认）和 Pi 的 50 KB，dsh 这里可能真的不截。
...
### A. Pi 子 agent：scout / implement 模板
...
### B. Codex `awaiter` 角色配置

`codex-rs/core/src/agent/builtins/awaiter.toml`（被注释掉，未启用，展示角色槽位怎么用）[^14]：
...
### C. Codex 子 agent 昵称生成

完整实现[^18]：

```rust
// codex-rs/core/src/agent/registry.rs:45
...
### D. Codex `SubagentStop` hook payload
...
### E. LangChain v1：子 agent 作为普通工具的完整测试
...
同一个测试里，消费 `run.subagents` 句柄的部分[^26]：
...
### F. DeepSeek Harness：子 agent preset 组合窗口
...
[^1]: Pi `packages/coding-agent/README.md:500`，Philosophy 一节「No sub-agents」原文。
[^2]: 同上，`README.md:508`，「No background bash」原文。
[^3]: `packages/coding-agent/examples/extensions/subagent/index.ts:294`，起子进程时拼装的四个参数。
[^4]: 同上，`index.ts:323`，系统提示词写临时文件后用 `--append-system-prompt` 传入。
[^5]: 同上，`index.ts:399`，`AbortSignal` 转 SIGTERM、5 秒后 SIGKILL 的实现。
[^6]: 同上，`index.ts:36`，`MAX_PARALLEL_TASKS`/`MAX_CONCURRENCY`/`COLLAPSED_ITEM_COUNT`/`PER_TASK_OUTPUT_CAP` 四个常量定义。
[^7]: `packages/coding-agent/examples/extensions/subagent/agents/scout.md` 全文，见附录 A。
[^8]: `packages/coding-agent/examples/extensions/subagent/agents/reviewer.md`，工具隔离评价原文。
[^9]: `packages/coding-agent/examples/extensions/subagent/prompts/implement.md` 全文，见附录 A。
[^10]: `codex-rs/models-manager/models.json:58`，`ultra` effort 档位描述。
[^11]: `codex-rs/core/src/session/multi_agents.rs:39`，`effective_multi_agent_mode` 函数实现。
[^12]: `codex-rs/core/src/context/multi_agent_mode_instructions.rs:6`，`EXPLICIT_REQUEST_ONLY_MULTI_AGENT_MODE_TEXT` / `PROACTIVE_MULTI_AGENT_MODE_TEXT` 两个常量原文。
[^13]: `codex-rs/core/src/agent/role.rs:359`，内置角色表与 `explorer` 描述原文。
[^14]: `codex-rs/core/src/agent/builtins/awaiter.toml`，全文见附录 B；`explorer.toml`（同目录）为 0 字节，`wc -c` 确认。
[^15]: `codex-rs/core/src/agent/role.rs:388`，`worker` 描述原文。
[^16]: `codex-rs/core/src/tools/handlers/multi_agents_common.rs:255`，`apply_spawn_agent_runtime_overrides` 里 `config.cwd = turn_cwd`。
[^17]: `codex-rs/core/src/agent/agent_names.txt`，100 个名字的列表。
[^18]: `codex-rs/core/src/agent/registry.rs:45`，`format_agent_nickname` 全文见附录 C。
[^19]: `codex-rs/core/src/session_prefix.rs:50`，`format_subagent_context_line` 函数。
[^20]: `codex-rs/core/src/context/inter_agent_completion_message.rs:35`，`body()` 方法的格式串。
[^21]: `codex-rs/core/src/session_prefix.rs:10`，四个 token 预算常量与 `ERROR_NEXT_ACTION` 原文。
[^22]: `codex-rs/core/src/config/mod.rs:284`，`pub(crate) const DEFAULT_AGENT_MAX_DEPTH: i32 = 1;`；并发闸门见 `AgentRegistry::reserve_spawn_slot`（`agents.max_concurrent_threads_per_session`）。
[^23]: `codex-rs/core/src/tools/handlers/multi_agents/spawn.rs:67`，深度超限的错误返回。
[^24]: `codex-rs/hooks/src/lib.rs:28`，`SubagentStart`/`SubagentStop` hook 声明。
[^25]: `codex-rs/hooks/src/schema.rs:592`，`SubagentStopCommandInput` 结构体，全文见附录 D。
[^26]: `libs/langchain_v1/tests/unit_tests/agents/test_subagent_transformer.py:33`，`test_subagents_surfaces_named_subagent` 全文见附录 E。
[^27]: `libs/langchain_v1/langchain/agents/factory.py:1795`，`create_agent(name=...)` 写入 metadata 的代码。
[^28]: 同上，`factory.py:1806`，`graph.compile(...)` 的 `transformers` 列表。
[^29]: `libs/langchain_v1/langchain/agents/_subagent_transformer.py:1`，模块 docstring。
[^30]: 同上，`_subagent_transformer.py:169`，`child_lc` 判定分支的注释。
[^31]: `libs/langgraph/langgraph/types.py:664`，`Send` 类 docstring。
[^32]: `libs/langgraph/langgraph/graph/state.py:212`，`input_schema`/`output_schema` 字段；`add_node` 的 per-node `input_schema` 见 `state.py:450`。
[^33]: `libs/sdk-py/integration/graph/deep_agent.py:1`，模块 docstring。
[^34]: Grok Build `crates/common/xai-tool-types/src/task.rs:14-110`，`TaskToolInput` 字段定义。
[^35]: 同上，`task.rs:21-30`，`run_in_background` 默认值与工具描述原文。
[^36]: 同上，`task.rs:55-56`，`isolation` worktree 模式描述原文。
[^37]: 同上，`task.rs:83`，`resume_from` 与 worktree 互斥。
[^38]: 同上，`task.rs:867-990`，三个内置 agent 类型。
[^39]: `xai-grok-agent/src/discovery.rs:186-189`，发现顺序原生认 `~/.claude/agents/`。
[^40]: `task/mod.rs:36`，`MAX_SUBAGENT_DEPTH: u32 = 1`。
[^41]: `subagent/mod.rs:52-63`，`InitialContextSource` 三档定义。
[^42]: `xai-grok-subagent-resolution/src/context.rs:11-48`，`Forked` 模式的规范化逻辑。
[^43]: `task.rs:210-253`，`SubagentCompletedOutput`。
[^44]: `task.rs:444-478, 656-714`，`get_task_output`/`wait_tasks`/`kill_task`。
[^45]: `hooks/src/event.rs:255-288`，`Stop` hook payload 里的 `backgroundTasks` 枚举。
[^46]: `xai-workflow/src/engine.rs:461-710`，host 函数与预算护栏；`DEFAULT_AGENT_BUDGET`/`MAX_HOST_CALLS` 定义在 `lib.rs`。
[^47]: dsh `docs/architecture.md:102`。
[^48]: `packages/subagent/README.md:11-16` 的 provider 表，逐个核对各包的 `registerProvider` 调用。
[^49]: 六个 provider 的字段坐标：`subagent-spawn-in-process/src/index.ts:42`（capabilities）/`:44`（inheritsParentContext）/`:55`（prepareContinuable）；`subagent-fork-in-process/src/index.ts:62`/`:64`/`:83`；`subagent-acp/src/index.ts:147`；`subagent-codex/src/index.ts:2-4`/`:49`；`subagent-claude-code/src/index.ts:2-4`/`:54`；`subagent-dsh-sdk/src/index.ts:94`。
[^50]: `packages/subagent/subagent/src/out-of-process.ts:19-23`。
[^51]: `packages/subagent/subagent/src/index.ts:442, 492`，`SubagentError('UNSUPPORTED_CAPABILITY')`。
[^52]: `packages/bundle/base/cordis.patch.yml:313-329`。
[^53]: `examples/acp-agent/product-subagent-both.cordis.yml:14-27`。
[^54]: `packages/subagent/tool-subagent/src/index.ts:211-236, :291`。
[^55]: `packages/workflow/tool-ralph/src/index.ts:228-230`。
[^56]: `subagent-fork-in-process/src/index.ts:48-53`，one-shot 路径调用见 `:68-69`。
[^57]: `packages/subagent/subagent/src/child-agent.ts:163-175`（组合窗口三件事的代码，见 `:168-174`）；四个内置 preset 见 `apps/cli/config/agent-presets/{code,standard,cordis,minimal}/agent.cordis.yml`；全文见附录 F。
[^58]: `child-agent.ts:108-111`。
[^59]: `child-agent.ts:199-215`。
[^60]: `child-agent.ts:135-139`。
[^61]: `packages/subagent/tool-subagent/src/index.ts:263`（`run_in_background` 默认）与 `:398`（`subagentId: started.childId`）。
[^62]: `packages/subagent/subagent/src/continuation.ts:145-159, 871-873`；`docs/subsystems/subagent.md:136`。
[^63]: `packages/subagent/tool-subagent-control/src/index.ts:82-87`。
[^64]: `packages/subagent/tool-subagent-report/src/index.ts:39-52`（注册点 `:64`，`reportDelivery` 选项见 `:26-36`）。
[^65]: `continuation.ts:83, 1414`；`docs/subsystems/subagent.md:212-231`。
[^66]: `packages/workflow/workflow-worker-thread/src/runtime.ts:100-108`。
[^67]: `packages/workflow/tool-workflow/src/index.ts:144`。
[^68]: `runtime.ts:39`，`SUPPORTED_AGENT_OPTIONS`。
[^69]: `runtime.ts:40-41`，`DEFERRED_AGENT_OPTIONS`。
[^70]: `workflow-worker-thread/tests/workflow-worker-thread.spec.ts:328-332`。
[^71]: `workflow-worker-thread/src/index.ts:35`，实现见 `:152`。
[^72]: `index.ts:116-121`；`runtime.ts:256-258`。
[^73]: `packages/workflow/tool-ralph/src/index.ts:413`（注册为 `ralph`），注释见 `:87-88`，循环体与 handoff 校验见 `:24-30, :90-177`。
[^74]: `packages/subagent/subagent/src/child-agent.ts:110`。
[^75]: `packages/subagent/subagent/src/out-of-process.ts:114-120`。
[^76]: `packages/subagent/tool-subagent/src/index.ts:98`，`maxDepth: … .default(3)`。
[^77]: `packages/subagent/subagent/src/depth.ts:35`。
[^78]: `packages/subagent/subagent/src/types.ts:280-284`。
[^79]: `packages/subagent/subagent/src/list-children.ts:32`。
[^80]: `packages/jobs/tool-jobs/src/index.ts:303, 343, 363`。
[^81]: `packages/subagent/tool-subagent/src/index.ts:406-422`。
[^82]: `packages/client/ui-subagent/README.md:5-11`，`SessionRuntime.openSubagent()`。
[^83]: `packages/subagent/tool-subagent/src/index.ts:219`（fork 措辞）、`:229-230`（spawn 措辞）。
[^84]: `docs/subsystems/subagent.md:316-323`；失败路径拼接见 `tool-subagent/src/index.ts:149-155`。
