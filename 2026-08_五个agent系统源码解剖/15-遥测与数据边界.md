...
这是本次调研在遥测维度上最重要的发现：端点和 API key 直接以字符串常量写死在源码里，照抄见附录 A[^1]。再看默认值——三个 exporter 里，logs 和 traces 默认 `None`，**唯独 metrics 默认指向 Statsig**，完整声明见附录 B[^2]。整条链路的门控逻辑长这样：
...
debug 构建直接短路返回、analytics 开关连带管住 metrics_exporter 的那两处判定，坐标见脚注[^3]。注意这个流程图里**没有"登录了吗"这个菱形**。Statsig 用的是硬编码的 client key，不是你的账号凭证——所以没登录也照发。
...
**唯一的关闭方式**是 `analytics.enabled = false`。注意这个配置项的名字里完全没有 "otel" 或 "metrics"，你从名字根本猜不到它同时管着 OTEL metrics 通道——analytics 关闭时 metrics_exporter 被强制置回 `None`[^3]。

### 3.2 analytics 事件 → ChatGPT 后端：脱敏认真，但有三处漏

第二条默认开的通道是 analytics 事件，POST 到 `{base_url}/codex/analytics-events/events`[^4]。和 metrics 不同，这条**需要登录**，而且登录方式决定发多少：
...
最后一条最能说明态度：指纹清空的地方有注释写明原因，原文照抄见附录 C[^6]。
...
| # | 什么 | 在哪 | 为什么算问题 |
|---|---|---|---|
| 1 | **git remote URL 明文** | `skill_invocation` 事件的 `repo_url` 字段，未哈希[^7] | 私有仓库的 remote URL 本身就是敏感信息：公司名、项目代号、有时甚至嵌着用户名 |
...
如果只查带 "telemetry / analytics / otel" 字样的代码，你会漏掉这条：仓库的 HEAD commit hash、git remote URL、有没有未提交改动，是并发取出后以仓库绝对路径为 key 塞进一个 `workspaces` 表的，完整片段照抄见附录 D[^8]。

这些信息被打包进一个叫 `x-codex-turn-metadata` 的 HTTP header，**随每一次模型请求**发往 OpenAI[^9]：
...
```
 ...
 │ x-codex-installation-id: <持久化设备 UUID>    │
...
也就是说即使你开了 logs 通道，用户 prompt 默认仍然被替换成占位符，判定片段照抄见附录 E[^10]。
...
记忆子系统（第 09 章）的用量遥测值得表扬一句：通过解析 shell 命令路径来统计记忆文件的使用，但它只发**5 种枚举分类 tag**（`MEMORY.md` / `raw_memories.md` / `skills/` 这种类别），不发路径本身，而且只解析通过了 `is_known_safe_command` 白名单的只读命令[^11]。
...
8. **在上传前显式清空的字段，留下注释说明为什么。** 附录 C 那三行是整个子系统里最值得抄的代码[^6]。
...
- **metrics 导出周期**：`export_interval` 默认 `None`[^13]，实际周期取决于 OTEL SDK 的默认值（60 秒），未在运行时抓包确认。
...
| 通道 | 默认 | 端点在哪 | 备注 |
|---|---|---|---|
| 产品事件 + Mixpanel | 关（`TelemetryMode::Disabled`）[^14] | `option_env!` 构建期注入，OSS 构建为 None | 明文的只有 Mixpanel host `api.mixpanel.com`（无 token 不触发） |
...
关键对照：Codex 的 metrics 端点和 client key 硬编码在源码里（第 3.1 节），任何人 build 都会发；**Grok 的一方端点在开源构建里全部拿不到**（`internal_defaults()` 返回全 None）[^15]——你自己 build 的版本发不出遥测。
...
即 **xAI 服务器可以在用户不改本地配置的情况下远程启用遥测**。对应 `resolve_telemetry_mode()`[^16]。
...
模型请求 header 里是 `x-grok-conv-id / x-grok-session-id / x-grok-agent-id / x-grok-user-id` 等 id 类字段——**不带仓库路径、不带 git remote URL**[^40]。仓库信息（`repo_root` 绝对路径 + remote URL）放在 GCS trace 的 `metadata.json` 里，而那条通道默认关。

正好和 Codex 互换：Codex 的仓库信息随每次模型请求外发且关不掉（第 4 节），设备标识却是随机 UUID；Grok 的仓库信息可关，设备标识却是**硬件派生指纹**——`x-grok-agent-id` 从 macOS 硬件序列号或 Linux machine-id 派生 UUIDv5[^41]，换配置、重装都跟着机器走。
...
- **清单第 4 条（按内容审计）**：内容上传路径被**编译期存根化**，不是配置开关——`upload_turn_messages` 只剩 `skip_artifact(..., "chat_content_upload_disabled")`，打包函数里赫然是 `let jsonl = { let _ = messages; Vec::new() };`，完整 prompt 上传同样只剩 `"prompt_content_upload_disabled"`[^42]。要恢复必须改源码重编译，在"禁用可信度"光谱上比任何 flag 都硬。配套的工作区分类器把 `$HOME` 本身、`~/Library`、Desktop/Downloads/Documents、`.ssh`/`.claude`/`.grok` 明确排除出上传范围[^43]，ZDR/opt-out 时还会主动 purge 本地待传队列。

文档披露也和 Codex 形成对照：顶层 README/SECURITY 没提遥测，但用户指南有专章，列出全部开关并讲外部 OTEL 的隐私模型[^17]——Codex 是仓库内 grep 零命中。
...
| `api.deepseek.com` | 模型调用与自带 web-search provider 都打这个 host[^18] | 用了才发 | 换 `baseURL` 或换 adapter |
...
`http://dsh.internal` 在非测试源码里出现 4 次，是 in-process RPC 桥用来 `new URL()` 的哨兵 origin[^19]，从不上网。

**Codex 的六套机制在 dsh 这里是一套。**

### 12.2 唯一那条通道：默认关，但打开就是全量原文

全仓唯一挂载遥测的地方是共享 base 配置里的一行（`dsh-base`，所有 profile 共用），mode 和 exporter URL 都读环境变量、默认落到 `'DISABLED'` 和厂商端点，原样照抄见附录 F[^20]。

代码侧默认同样是关：`DEFAULT_TELEMETRY_MODE = SessionTelemetryMode.DISABLED`，且 `DISABLED` 分支在构造函数里**直接 return，不构造 LoggerProvider / processor / exporter**[^21]——不是"构造了但不发"，是管道根本不存在。
...
只承认**已经落进 `session.events[event.seq]` 的那个对象本身**[^23]。

这是本章清单第 3 条"两道闸"的一个新变体——第二道闸不是配置项，是**一个必须由人做出的动作**。

代价写在同一份代码里：捕获时 `body: structuredClone(event.data)` 是事件数据的完整深拷贝，identity 属性里还带 `session.cwd` 绝对路径[^24]。

脱敏是一个 waterfall 扩展点，而 **seam 自己不带任何规则**——JSDoc 原文是 `It ships NO rules of its own`[^25]，全仓也没有任何 bundle 挂过 `session-telemetry/record` 监听器（唯一的独立实现文件是一份测试夹具）。所以打开 = 原样上传。

### 12.3 最重要的一条：默认是开源前三天才翻过来的

翻这份配置里那个环境变量名的 git 历史，只有一个提交动过它：`e6eb44ed2a`，**2026-08-10 17:45:50 +0800，"feat(telemetry): require explicit opt-in"**[^26]。它删掉的那段注释逐字是：
...
Codex 第 4 节的同位物在 dsh 这里存在，但内容窄得多。每次模型请求的 header 里带着一个匿名 UUID 用户 id、恒发，完整片段照抄见附录 G[^27]。

**不带**仓库绝对路径、**不带** git remote、**不带** commit hash——Codex 的 `x-codex-turn-metadata` 里那四样在 dsh 一样都没有。

但同样关不掉，而且仓库里把这件事写死成了决策：Agent Note 的"已否决备选"表里有一行 `Disable the header with telemetry sharing`，理由是 "one switch would hide the actual privacy boundary"；`DSH_TELEMETRY_DISABLED` 的说明文档直白写着它 "does not suppress direct feedback acknowledgement or the DeepSeek provider header"；`attributionHeaders()` 的 JSDoc 也写 "omission cannot suppress attribution"[^28]。
...
**匿名 id 是纯随机 UUIDv4**，落 `$DSH_HOME/.anonymous-user-id`，删文件即换身份；模块头注释逐字写 "never derived from the hostname, network address, git remote, or any other identifying source"[^44]。
...
**Web UI 里点赞点踩什么都不发**：`message-feedback` 是 storage-domain 本地 sidecar，原文 "it performs no telemetry handoff"，另一处文档再确认一次"never enters … the model context, or telemetry"[^30]。

会触发上传的是**另一个东西**：`/feedback <text>` 命令，它 append 一条 `feedback/record` 事件，在 `FEEDBACK_ONLY` 下就是那把钥匙[^31]。而且这个命令的回执**当场把当前策略念给用户听**：`Session sharing is disabled.` / `…is feedback-gated; recording feedback releases the session prefix for sharing.` / `Session sharing is not configured.`[^32]——**知情同意做在了触发动作的返回值里，本章其余四家未见同类做法**。

**本地日志无加密**，磁盘上的保护只有文件权限这一层：

| 落盘物 | 位置 | 权限 |
|---|---|---|
| 会话 JSONL | `$DSH_HOME/sessions` | 目录 `0o700`、文件 `0o600`[^33] |
| 凭据（明文 YAML） | `$DSH_HOME/.credentials.yaml` | `0o600`，权限不对会拒绝启动并提示 `chmod 600`[^34] |
...
**凭据进不进日志**：adapter 的 API key 是构造函数参数而非 session event，结构上不在日志里，这一点文档有明确陈述且与代码一致[^35]。但子进程 env 的凭据过滤是**名字启发式**，仓库自己承认漏：`*KEY*`/`*PASSWORD*`/`*SECRET*`/`*TOKEN*` 之外（例如 `*PASSPHRASE*`）直接透传[^36]——而命令输出会完整进 session log，`FULL` 模式下就会外发。

### 12.6 披露、治理，与文档-代码不一致

**披露是五家里最好的**，而且是唯一把"会外发什么"写成清单的：包 README 里整段 "What leaves the machine" 逐项点名——用户与 assistant 消息内容、工具参数与结果（命令输出、文件内容）、完整 system prompt 与 tool schema、todo 文本、压缩摘要、hook `stderrSummary`、反馈文本、`cwd` 本地路径；CLI reference 再给一遍环境变量三件套[^37]。对照 Codex 的仓库内 grep 零命中（第 6 节）和 Grok 的用户指南专章（第 11.4 节），dsh 把披露写进了包 README。
...
| # | 不一致 |
|---|---|
| ① | 架构文档把 capability events 列作 `fs/*`、`tools/*`、`telemetry/*`，而代码里**不存在任何 `telemetry/*` 事件**，真实事件名是 `session-telemetry/record`[^38] |
| ② | 包 README 把包名写成 `@deepseek-ai/dsh-session-sessionTelemetry-otel`，与 `package.json` 里的真名 `@deepseek-ai/dsh-session-telemetry-otel` 对不上[^39] |
...
```rust
// analytics/src/accepted_lines.rs:123-125
...
### D. `x-codex-turn-metadata` 的组装片段

```rust
// core/src/turn_metadata.rs:370-392
...
```rust
// otel/src/events/session_telemetry.rs:968-971
...
```yaml
# packages/bundle/base/cordis.patch.yml:148-161（挂载段落起点），实际字段在 :151,153-154
...
### G. dsh 模型请求 header 组装

```ts
// packages/llm/llm-deepseek/src/adapter.ts:283-295
...
[^1]: `otel/src/config.rs:9-11`：Statsig 端点、header 名、client key 三个常量的声明。全文见附录 A。
[^2]: `config/src/types.rs:584-593`：`OtelConfig` 的 `Default` 实现，logs/traces 默认 `None`、metrics 默认 `Statsig`。全文见附录 B。
[^3]: debug 构建短路：`otel/src/config.rs` 内 `if cfg!(debug_assertions) { return None; }`；analytics 开关连带管住 metrics_exporter：`core/src/otel_init.rs:70-77`。
[^4]: analytics 事件的上传端点：`analytics/src/client.rs:118`（`POST {base_url}/codex/analytics-events/events`）。
[^5]: 登录判定与两类事件白名单：`analytics/src/client.rs:645-655`。
[^6]: `analytics/src/accepted_lines.rs:123-125`：清空代码行指纹前的注释与赋值。全文见附录 C。
[^7]: `skill_invocation` 事件的 `repo_url` 字段（未哈希）：`analytics/src/reducer.rs:1073-1102`。
[^8]: `core/src/turn_metadata.rs:370-392`：HEAD commit hash / remote URL / 未提交改动并发取出，以仓库绝对路径为 key 组装进 `workspaces`。全文见附录 D。
[^9]: `x-codex-turn-metadata` header 的注入点：`core/src/client.rs:145`；同一处组装的 `x-codex-installation-id`（持久化设备 UUID）：`core/src/client.rs:143`。
[^10]: `otel/src/events/session_telemetry.rs:968-971`：`log_user_prompts` 为假时把 prompt 替换成 `"[REDACTED]"`。全文见附录 E。
[^11]: `memories/read/src/usage.rs:29-63`：解析只读 shell 命令、只上报 5 种枚举分类 tag 的用量遥测实现。
[^12]: `log_event!` / `trace_event!` 两个宏的定义：`otel/src/events/shared.rs`。
[^13]: `export_interval` 默认值：`otel/src/metrics/config.rs:55`。
[^14]: Grok 产品事件默认关闭：`TelemetryMode::Disabled`，`config.rs:2350`。
[^15]: Grok 开源构建下端点/token 全部为 `None`：`internal_defaults()`，`telemetry/config.rs:119-139`。
[^16]: Grok 远程配置优先于本地默认的解析函数：`resolve_telemetry_mode()`，`config.rs:2340-2367`。
[^17]: Grok 用户指南的遥测披露专章：`docs/user-guide/05-configuration.md:482-508`（全部开关）、`24-monitoring-usage.md`（外部 OTEL 隐私模型）。
[^18]: dsh 模型调用与内置 web-search provider 打 `api.deepseek.com`：`llm-deepseek/src/index.ts:104`、`web-search-deepseek/src/provider.ts:32,35`。
[^19]: `http://dsh.internal` 哨兵 origin 的两处使用点：`packages/client/connection/src/client/rpc.ts:11`、`http-bridge.ts:69`。
[^20]: dsh 唯一的遥测挂载点：`packages/bundle/base/cordis.patch.yml:148-161`，`mode` 与 `exporter.url` 字段在 `:151,153-154`。全文见附录 F。
[^21]: `DEFAULT_TELEMETRY_MODE = SessionTelemetryMode.DISABLED`：`session-telemetry-otel/src/index.ts:51`；`DISABLED` 分支直接 return、不构造 processor/exporter：同文件 `:159-167`。
[^22]: 三档 mode（`FULL` / `FEEDBACK_ONLY` / `DISABLED`）的行为差异：`session-telemetry-otel/src/index.ts:237-252`。
[^23]: `FEEDBACK_ONLY` 的同一性判定（只认 `session.events[e.seq]` 那个对象本身）：`session-telemetry-otel/src/index.ts:247-248`。
[^24]: 捕获时对事件数据做完整深拷贝：`session-telemetry/src/coordinator.ts:199`；`identity` 属性携带 `session.cwd` 绝对路径：同文件 `:313`。
[^25]: 脱敏 seam 不自带规则的 JSDoc：`session-telemetry/src/index.ts:27`；事件签名行：同文件 `:43`；唯一的独立实现（测试夹具）：`examples/headless-agent/tests/fixtures/telemetry-redact-rule.ts`。
[^26]: 把遥测默认值从开翻转为关的提交：`e6eb44ed2a`，改动文件 `packages/bundle/base/cordis.patch.yml`。
[^27]: dsh 模型请求 header 组装：`packages/llm/llm-deepseek/src/adapter.ts:283-295`。全文见附录 G。
[^28]: 已否决备选表：`.agents/notes/implemented/feature/2026-08-11-deepseek-request-user-id-header.md`；`DSH_TELEMETRY_DISABLED` 说明文档：`packages/identity/anonymous-user-id/README.md:15`；`attributionHeaders()` 的 JSDoc：`packages/llm/llm/src/attribution.ts:61`。
[^29]: `.anonymous-user-id` 文件的 lazy 创建时机：`adapter.ts:221-222`。
[^30]: `message-feedback` 本地 sidecar 说明：`docs/subsystems/feedback.md:5`；再次确认不进遥测：`packages/client/ui-message-feedback/README.md:15`。
[^31]: `/feedback` 命令的实现：`packages/feedback/command-feedback/src/index.ts:75`。
[^32]: 回执文案三行：同文件 `:27-39`、`:51`、`:95`。
[^33]: 会话 JSONL 目录：`cordis.patch.yml:101`；文件/目录权限设置：`session-persistence-jsonl/src/index.ts:536-540,608`。
[^34]: 凭据文件权限校验与拒绝启动逻辑：`credentials-local/src/index.ts:119,394`。
[^35]: API key 不进日志的陈述：`session-telemetry-otel/README.md:38`。
[^36]: 子进程 env 凭据过滤的名字启发式说明：`packages/subprocess/subprocess-local/README.md:32`。
[^37]: "What leaves the machine" 清单：`packages/session/session-telemetry-otel/README.md:36-38`；环境变量三件套：`apps/cli/reference/README.md:78`。
[^38]: 架构文档的 capability events 分类：`docs/architecture.md:59`；真实事件名：`session-telemetry/src/index.ts:43`。
[^39]: 包 README 里的错误包名：`session-telemetry-otel/README.md:5,11`；真名见 `package.json:2`。
[^40]: Grok 模型请求 header 的 id 类字段：`sampler/client.rs:46-77`。
[^41]: `x-grok-agent-id` 的硬件序列号派生 UUIDv5：`telemetry/id.rs:37-66`。
[^42]: `upload_turn_messages` 打桩：`upload/trace.rs:948-959`；消息丢弃、返回空 Vec 的打包函数：同文件 `:990-993`；prompt 上传打桩：同文件 `:439-445`。
[^43]: 工作区分类器排除范围：`xai-file-utils/src/workspace_classifier.rs`。
[^44]: 匿名 id 模块头注释：`packages/identity/anonymous-user-id/src/index.ts:6,19,29`。
