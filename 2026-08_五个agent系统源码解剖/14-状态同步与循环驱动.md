...
你在上一轮对话里说的"LangChain 用 status 同步状态"，指的就是第三种。严格说不是 status 字段，而是**每个 channel 的版本号**（`channel_versions`）和**每个节点见过的版本号**（`versions_seen`）之间的差值在驱动调度——比较发生在 `pregel/_algo.py:609` 附近，`:263` 处任务跑完就把 `versions_seen` 对齐。
...
| LangChain | `channels/last_value.py:61` | 一个没配 reducer 的字段在同一步收到两次写入 → `InvalidUpdateError: "Can receive only one value per step. Use an Annotated key to handle multiple values."` |
...
**分支几乎免费。** 状态本来就是棵树，每条 entry 带 `parentId`，追加时写的是 `parentId: await this.storage.getLeafId()`（session.ts:223 起连续七处都是这个写法）。

想"回到第 3 步重来"？把 leafId 指针指回 entry₃，往下追加的新 entry 自然长成新分支。另外两家都得为分支单独造机制（见第 2.4 节和第 10 章）。

**崩溃安全几乎免费。** `_persist()` 用的是**同步的** `appendFileSync`，每条 entry 立即落盘、不缓冲不批量（coding-agent `session-manager.ts:1015-1021`）。进程被 `kill -9`，最多丢正在写的一条。
...
**同步写盘在主线程上。** Node 单线程，`appendFileSync` 是实打实的阻塞。注意分层：agent 包内部的 `jsonl-storage.ts:266` 走的是异步 `appendFile`，但产品层 session-manager 是同步的。大量小写入时会卡 UI。
...
**代价一：内存是真相源 → 崩溃即丢，只能靠重放。** 磁盘上的 rollout 文件（第 10 章）是事后追记的日志，重启后要靠 `reconstruct_history_from_rollout`（`rollout_reconstruction.rs:113`）把内存状态重建出来。
...
**代价三：`history_version` 是弱通知。** 版本号 bump 了（`history.rs:46,159`），但持有旧 Arc 快照的消费者**不会被主动叫醒**，得自己记得比对版本——容易漏检。
...
用户中途插话？不像 Pi 那样排进 steering 队列等当前轮完——Codex 直接 `abort_all_tasks(Replaced)`（`tasks/mod.rs:282`）把旧任务掐了换新的。状态推进权永远只在一个任务手里，这就是它不需要 reducer 的原因。
...
**可确定性重放。** 归约顺序按 `task.path` 排序而不是按完成时间（`_algo.py:256`），意味着同样输入必然同样输出——不管这次运行里哪个节点碰巧先跑完。

对写测试和查"为什么昨天那次跑出了不同结果"是巨大优势。Pi 重放的是日志（忠实但只能重放发生过的），Codex 重放靠一段会漂移的重建代码，都做不到"重新执行也保证一致"。

**最细粒度的崩溃恢复。** `put_writes`（`checkpoint/base/__init__.py:300`）把**单个任务**完成的写入先记下来——superstep 跑到一半宕机，重启后已完成任务的写入直接复用，只重跑没完成的。
...
**抽象在泄漏，有实锤。** `factory.py:1794`：`config: RunnableConfig = {"recursion_limit": 9_999}`——因为中间件节点让"图步数 ≠ agent 轮数"，用户设的循环上限失去意义，框架索性把上限 hack 成 9999。
...
消息历史的唯一 owner 是 `ChatStateActor`——一个独立的 tokio task，所有变更都是 `ChatStateCommand` 经 mpsc 通道送达、在 `handle_command` 里**串行**执行：
...
出处 `xai-chat-state/src/actor/mod.rs:28-43, 97-114`。crate 门面上的 ASCII 架构图直接写着 "State (no locks needed)"（`xai-chat-state/src/lib.rs:12`）；持久化 trait 拿 `Box<dyn ChatPersistence>` 独占所有权、方法全是 `&mut self`，注释原话："no locks, no atomics, no shared state"。
...
读者视图那格值得停一下：`GetConversation` 直接 `self.state.conversation.clone()`，还打了一条 "cloning full conversation" 的 debug log 自认成本（actor/mod.rs:305-311）。
...
**Grok Build 用的恰恰就是 `FuturesUnordered`**（tool_calls.rs:611）——并行工具结果按**完成顺序**回灌历史，不保证请求序。它没有乱序 bug，因为正确性根本不靠序，靠三件套：
...
两种哲学的取舍很清楚：Codex 的防患于未然依赖那个类型名永远没人改错；Grok 的宽进严出多付一份修复代码，但对乱序、崩溃、abort 三种成因（mutations.rs:29-47 把三种都列了）用同一套机制兜底。

并行竞态防护则是参数级的：同批次非只读调用命中同一 `file_path` 时共享 per-path `tokio::sync::Mutex` 按模型发出顺序串行，其余全并发（tool_dispatch.rs:50-59）。
...
stationarity 的具体阈值：连续相同 (tool, args) 签名 8 次注入 nudge、16 次硬停、`run true` 式空转 4 次即停（turn.rs:2724-2728，常量间还有编译期断言），硬停返回专用 `TurnOutcome::StationarityEnded`，注释明确与 `Completed` 区分"防止恢复逻辑重开循环"。

插话是双范式并存：默认 Pi 式排队（"An interjection never cancels the turn"，interjection.rs:332），另留 `send_now` 的 Codex 式 cancel-and-send。
...
对应 `packages/core/agent-loop/src/agent.ts:172-193`。phase 是三态机 `idle | maintenance | running`（`agent.ts:38-46`），唯一会因并发而拒绝调用方的是维护任务：`agent "…" already has active work`（`agent.ts:143`）。
...
代价写在官方防御模式文档里，措辞很硬：**"Async state is not synchronous state"**——`agent.followup()` 没有 per-message 完成信号，多条排队消息、steering、注入工作可能共享同一段 `running` 区间，所以调用方"必须显式定义自己的区间，并把选中的输出描述为区间级的、而不是因果归属到那条消息"（`docs/defensive-patterns.md:17`）。
...
同一个监听器还要求请求对象被 frozen、messages 数组被 frozen、日志里必须有 `step/start`、`request/header` 折叠出的 model/system/temperature/maxTokens/stop/tools 逐项与实际请求相等（`invariant.ts:22-52`）。

它用 `{ global: true, prepend: true }` 挂在 `llm/stream` 最前面，注释原话：**"Prepend prevents a short-circuiting replay listener from silencing the check."**（`invariant.ts:20`）

**一处文档口径与代码覆盖面的差**，这是本节最该记下的发现。`AGENTS.md:107` 的表述是全称的——"anything that reaches a model request must be reconstructable from the session log"。

但断言的第一行是 `if (!isAgentLoopRequest(options)) return next()`，而 `isAgentLoopRequest` 查的是一个 `WeakSet`，只有 agent-loop 亲手 `markAgentLoopRequest()` 的那个对象在里面（`packages/llm/llm/src/call-config.ts:13,66-78`）。
...
它不收紧写者，而是加厚写入函数。一次 `append()` 要过四道闸：
...
出处 `packages/core/session/src/index.ts:604-655`。类型层还强制：只有 `user/message` / `assistant/message` / `tool/result` 三种"产消息"事件**必须**声明 `surfaceOp`，其余类型写了会编译报错（`packages/core/session/src/types.ts:343-347, 404-435`）。
...
session 包自己那份就是一台事件日志状态机：seq 必须严格递增、`turn/start` 不能套 `turn/start`、`step/end` 时 `pendingCalls` 清空、`tool/result` 必须能在本 step 找到对应 `tool/call`（`packages/core/session/src/invariant.ts:60-161`）。

它挂在 `internal/dispatch` 上做**提交前**校验、在 `session/event` 上做提交后推进，注释写明"A later dispatch listener may veto. Validation is pure…"（`invariant.ts:233-241`）。
...
`get events()` 返回 `this.eventsSnapshot ??= Object.freeze([...this.log])`，下次 append 才作废（`index.ts:559-562`）——等于用 JS GC 白嫖了 Codex 的 `Arc` 引用计数，而且事件在 acceptance 时已深冻，注释原话："neither a cast nor ordinary JavaScript can rewrite durable history"。

`deriveMessages()` 更进一步：**每个 surface 节点只投影一次**，一次调用只花 O(新节点)，压缩触发的 surface 重写靠 `replaceGeneration` 整体失效重建（`index.ts:726-747`）。
...
`Inbox` 构造时扫 `session.events` 里 seed 之后的部分，重放其中全部 `agent/inbox/spliced`（`packages/core/agent/src/inbox.ts:32-39`）。

此后每次增删都先 `session.append('agent/inbox/spliced', splice)` **再**改内存投影，注释点明顺序的用意："The durable event commits before the live projection mutates, so synchronous `session/event` observers see the pre-splice lists"（`inbox.ts:128-132, 186-187`）。

驱动条件在 `agent.ts:212`（`while (await this.turn()) {}`）、`agent.ts:324`（`if (!this.inbox.hasPending) return false`）、`agent.ts:295-299`（内层 step 续跑条件）。
...
热路径**不落盘**：`append()` 只写内存并同步派发，"The hot path never blocks on I/O — persistence plugins buffer asynchronously"（`index.ts:571-572`），write-behind 默认批窗 `DEFAULT_WRITE_BATCH_MAX_DELAY_MS = 200`（`packages/session/session-persistence/src/coordinator.ts:30`）。

真正的耐久性来自**三个 fail-closed 的语义检查点**，全在 `packages/session/session-checkpoint-policy/src/index.ts`：

| 检查点 | 卡在哪 | 行号 |
|---|---|---|
| `llm/stream` | 模型请求发出前 flush，日志前缀不落盘就不给适配器 | `:64-68` |
| `tools/execute` | 顶层工具体执行前 flush，flush 后若已 abort 则返回 `tool call aborted before dispatch` | `:70-75` |
| `agent/pre-step` | 每步开始前把上一步提交的一切落盘 | `:79-82` |
...
落盘端也是真 fsync：`handle.writeFile(content)` 后 `handle.sync()`，部分写或 sync 失败就 `truncate` 回滚到写前大小再抛，"leaving partial bytes would create duplicate sequence numbers"（`packages/session/session-persistence-jsonl/src/index.ts:651-689`）。

同仓库里另一条写路径则明说不做：`writeFileAtomic` 的 JSDoc 写着 "Crash durability (fsync) is out of scope."，下面还挂着 `TODO(settings-atomic-durability)`（`packages/util/atomic-write/src/index.ts:44,54`）——**同一个仓库里两种耐久等级，会话日志是一等，设置文件是二等**。

resume 侧是确定性修复而非猜测：`interruptedTurnClosers()` 扫日志，给没配对的 tool call 补 `TOOL_NOT_STARTED` / `TOOL_OUTCOME_UNKNOWN` 错误结果，再补 `step/end` 和 `interrupted` 的 `turn/end`（`packages/core/session/src/repair.ts:13-16,27`；调用点 `coordinator.ts:903,981`）。

合成 closer 本身也是日志事件，所以 8.3 那台状态机断言照样能验它。

### 8.7 两个四家都没有的角度：effect 回滚，和 HMR 中途换插件

**注册也是状态。** Cordis 的 `ctx.effect()` 收集所有 disposer，**倒序**执行，重复调用是 no-op，fiber 卸载时自动跑（`vendor/cordis/src/fiber.ts:402-442`）。

插件卸载即回滚它注册过的 prompt section、tool schema、adapter、监听器——这是本章四家都没触及的一类"状态"：不是消息，是**运行时装配**。

配套纪律在 `docs/defensive-patterns.md:19-21`：**"Dispose must reach quiescence, not just request it"**——teardown 必须 kill 后 await 子进程 `done`，且**先关监听器/通知注册表再 kill**，让迟到的完成静默。
...
注释写明理由——"that crash-repairs open turns as interrupted, which is wrong for HMR while the live Session is still the authority and may append the real step/turn end later"（`packages/session/session-persistence/src/coordinator.ts:1272-1323`）。
...
dsh 把其中一半做成了**类型层字段**：surface 事件可带 `sourceEventSeqs`——"Seq numbers of earlier events that this event cites as sources"，压缩的 `replace` 节点**必须**列全被它遮蔽的每一个 surface 节点，否则 append 直接失败（`packages/core/session/src/types.ts:359-389, 423-434`）。

这是"why / from-what"，可回答"这条消息是被哪次压缩吃掉的"。

它是真的被当证据用的：postmortem 0003 在复盘一次 Web agent 走错验收目标时，直接引用会话日志的 seq 号做时间线——"Its initial request header is sequence 6; the user-facing handoff, bare-Vite launch, replacement-host launch, boot-manifest probe, and first 3081 process probe are sequences 30939, 31865, 34309, 34441, and 34681 respectively. The timeline below follows those events rather than reconstructing intent from the later report."（`docs/postmortem/0003-web-agent-gui-feedback-loop.md:17`）
...
- ⚠️ 不变式注册表默认 `enabled: true`（`docs/subsystems/invariants.md:14-15` 的 Config 声明），但**生产 profile 是否真的开着、8.2 那条比对的 CPU 开销**（每步两次 `JSON.stringify` 全量历史）未实测，长会话下的代价未知。
