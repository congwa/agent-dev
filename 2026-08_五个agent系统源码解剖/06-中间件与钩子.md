...
文档原文的警告没打折扣：「Extensions run with your full system permissions and can execute arbitrary code. Only install from sources you trust.」出处 `docs/extensions.md:109`。

33 个事件按阶段分组如下——这张表就是完整清单，因为 `pi.on()` 的重载签名本身就是清单（`src/core/extensions/types.ts:1198-1239`）：
...
这在三家里是独一份的：你可以完整替换掉发给 provider 的 JSON body。Pi 的 `examples/extensions/custom-provider-anthropic` 和 `provider-payload.ts` 就是靠这个做的。
...
`docs/extensions.md` 有 2987 行、是全仓最长的文档，这不是巧合——文档里甚至要专门解释「在默认并行工具执行模式下，同一条 assistant 消息的兄弟工具调用会先串行预检、再并发执行，所以 `tool_call` 不保证能在 `ctx.sessionManager` 里看到兄弟工具的结果」这种细节。
...
它是 `Stage::Stable`、`default_enabled: true`，配置 key 为 `hooks`，legacy 别名 `codex_hooks`（`features/src/lib.rs:1037`）。
...
配置格式是 `事件名 → MatcherGroup[] → HookHandlerConfig[]`，`matcher` 是正则，`command` 是 shell 字符串（`codex-rs/config/src/hook_config.rs:35`、`:140`、`:147`）：
...
2 是 deny 的专用码，不是"随便一个非零值"——同族的 Grok Build 干脆给它起了个名字，`GATE_EXIT_CODE = 2`（`runner/command.rs:19`，详见第 8 节）。

超时的默认值分两档：`SessionEnd` 默认 1 秒、上限 3 秒，其他 hook 默认 10 分钟。出处：解析逻辑见 `hooks/src/events/pre_tool_use.rs:200-290`，超时见 `hooks/src/engine/command_runner.rs:101`。
...
值得一提的是，这个开关**只在 requirements 层（MDM / 企业配置）生效，写在普通 `config.toml` 里不算数**——测试 `allow_managed_hooks_only_in_config_toml_does_not_enable_policy`（`hooks/src/engine/mod_tests.rs:782`）明确固定了这个行为。
...
而且 hook 只能看到 JSON payload 里给的字段——`tool_name`、`tool_input`、`cwd`、`transcript_path` 等（`hooks/schema/generated/pre-tool-use.command.input.schema.json`）。想读完整会话？只给一个 `transcript_path` 让你自己去解析文件。
...
基类的钩子清单如下，每个都有 `a` 前缀的 async 版本（`langchain/agents/middleware/types.py:383` 起）：
...
`except GraphBubbleUp: raise` 是必须的细节——LangGraph 的控制流信号（中断、父级命令）不能被当成「模型失败」吞掉重试。`ToolRetryMiddleware.wrap_tool_call`（`tool_retry.py:290`）里有一模一样的模式，注释写着「Control-flow signals (interrupts, parent commands) must propagate, not be retried or converted to error messages.」
...
有趣的是两边的分歧不在机制而在**判断**：Pi 说 to-do 让模型混乱，LangChain 的 `WRITE_TODOS_SYSTEM_PROMPT` 里恰好花了大段文字在对抗同一个混乱（`todo.py:117`）——「`write_todos` tracks your work; it does not deliver the answer... Marking the last todo complete is not itself an answer to the user.」
...
**洋葱层之间不是真解耦。** `model_fallback.py` 顶部那段关于 `cache_control` 的 docstring 是最好的证据：中间件 A 的行为（打缓存标记）在中间件 B 重试时会失效，而 B 不得不硬编码 A 的知识去清理。
...
**形态**：两种 handler，`HandlerType::{Command, Http}`（`xai-grok-hooks/src/config.rs:145-148`）。

Command 与 Codex 同构：spawn 子进程、事件 envelope JSON 写 stdin、读 stdout JSON + exit code，deny 专用 `GATE_EXIT_CODE = 2`（`runner/command.rs:19`），裁决优先级 stdout JSON > exit code——即第 1 节说的"「能改」和「能中止」分通道"，它和 Codex 选了同一套。

Http 是本章三个主样本都没有的：POST 到 URL 的 webhook，带 SSRF 校验（`runner/http.rs`）——把 hook 从"本机脚本"扩展到了"团队服务端策略"。
...
失败策略一律 **fail-open**（超时/崩溃/坏输出不拦截，`dispatcher.rs:48-59`）。对照第 6 节工程经验第 2 条：Pi 给权限类拦截点选了 fail-closed（`emitToolCall` 不包 try/catch），Grok 全线 fail-open，这意味着**它的 hook 是旁路策略层，不是安全边界**——安全边界在 permission 引擎和沙箱（见 [11 章](./11-安全沙箱与权限.md)第 8 节）。

**第二平面**：除用户 hooks 外另有内部 workspace 的 turn hook（`xai-tool-protocol/src/turn_hook.rs:137-203`），`before_turn`/`after_turn` 请求-响应式，`HookReply` 可注入 System/Developer/User turn 并 `ForceContinue`/`ForceStop` 循环——能力比用户 hooks 强一档，但那是内部协议不对用户开放。
...
vendored cordis 的 waterfall 只有十行（`vendor/cordis/src/events.ts:234-243`）：
...
| 短路 | 不调 `handler` 直接 return | 同（`events.ts:80` 原文：`not calling it vetoes`） |
...
第二条我按 `events.ts:234-243` 原样复刻跑了一遍（两个监听器 A/B + terminal，A 调两次 `next()`）：输出是 `A-in | B | TERMINAL#1 | A-mid | TERMINAL#2 | A-retry`——**terminal 确实跑了两次，但 B 在重试时被整条跳过了**。
...
`packages/llm/llm/src/index.ts:53` 把 `llm/stream` 描述为 "Waterfall around every streaming model call (**retry**, replay, routing)"，`packages/core/tools/src/index.ts:154` 把 `tools/execute` 描述为 "Around-dispatch waterfall for timeout, **retry**, or metrics"，`docs/cookbook/extension-cookbook.md:114` 同样写 "Tool deadline / retry / metrics"。

但按上面的游标语义，一个真靠二次 `next()` 重试的插件会**静默跳过同一 waterfall 上的兄弟监听器**——`llm/stream` 现有 5 个监听包、`tools/execute` 现有 2 个（`docs/event-producer-consumer.md` 矩阵）。
...
重试循环是 loop 自己的 `while (true)`（`packages/core/agent-loop/src/agent.ts:339`）；流跑完发现 `finish.kind` 是 `'error'` 或 `'aborted'` 时（`agent.ts:354`），loop **把决定权 waterfall 出去**：
...
`RequestErrorAction = { kind: 'retry' } | undefined`（`packages/core/agent/src/runtime-types.ts:58`）——整个协议只有一个 bit。

`continue` 之后重新进 `buildRequest()`，于是**又走一遍 `agent/request` waterfall**（`agent.ts:438-441`），而 `agent/request` 正是换模型的地方（`runtime-types.ts:244`）：
...
现成范式在 `packages/core/agent/src/model-selection.ts:54-70`：`const resolved = await next()`，然后返回 `{...resolved, provider, model}`。
...
重试执行器本身已经在仓库里：`packages/llm/llm-retry`（src 共 494 行，主文件 `index.ts` 226 行）。
...
Pi 在本章是唯一能下探到 HTTP 层改 payload 的（`before_provider_request`）。dsh 明确**反着来**：`agent/request` 的文档写着「Model-visible content must use logged channels; this waterfall cannot mutate messages.」（`runtime-types.ts:235-236`），`llm/stream` 拿到的 loop 请求是 deep-frozen 的（`llm/src/index.ts:57-59`，原文 "arrives deep-frozen (mutation throws) … so listeners read it, never rewrite it"）。
...
invariant 注册时带 `{ global: true, prepend: true }`，注释解释得很直白：「Prepend prevents a short-circuiting replay listener from silencing the check.」出处 `packages/core/agent-loop/src/invariant.ts:20-54`。
...
定位写得很清楚（`packages/hooks/README.md:5`）：「a "native hook" is just an ordinary Cordis plugin on those extension points. These packages are the **bridges** that translate the external shell-hook protocol onto that same surface.」

**外部进程钩子在 dsh 里不是一等公民，是兼容层**。这是五个样本里第一次出现这种权力反转（Codex/Grok 的 hook 就是钩子机制本身）。

| 维度 | dsh 的 Claude Code 桥 | 对照 Codex / Grok |
|---|---|---|
| 支持事件 | **7 个**：SessionStart、UserPromptSubmit、PreToolUse、PostToolUse、Stop、SubagentStart、SubagentStop（`hooks-claude-code/src/index.ts:206/219/238/247/270/281/291`）；Codex 桥只有 **5 个**（`hooks-codex/src/index.ts:1-6`） | Codex 11、Grok 14 |
| 改工具参数 | **不支持**：`updatedInput` 被解析、记录、warn，然后丢弃（`hooks-claude-code/src/index.ts:175-177`，模块头原文 "`updatedInput` is logged and warned but not honored"） | Codex 支持；Grok 同样不支持 |
| 裁决通道 | stdout JSON（`permissionDecision`）+ exit 2 + stderr，两条通道；顶层 `decision` 只认 `approve`/`block`，越权的 `{"decision":"deny"}` 被**明确忽略**（`hook-protocol/src/codec.ts:33-38`） | 与 Codex/Grok 同族 |
| 多 hook 合并 | **串行 for 循环**（`index.ts:152-185`）+ `deny > ask > allow` 单调折叠（`hook-protocol/src/merge.ts:1-6`） | Codex 是 `FuturesUnordered` 并发、改写冲突取完成最晚者（非确定）——dsh 这里是确定的 |
| 失败策略 | 全线 **fail-open**：spawn 失败 / 无退出码 / 基础设施异常都转成"非阻塞错误"（catch 块在 `hook-protocol/src/runner.ts:96-105`），`runHook` 的 JSDoc 承诺 "never throws or crashes the calling turn"（`runner.ts:60`） | 与 Grok 一致；Pi 的 `tool_call` 是 fail-closed |
| 超时 | 默认 600000ms，注释说明这就是 CC 和 Codex 共同的参考默认值（`runner.ts:13-20`） | 同 Codex |
| 信任模型 | **无 hash 签名、无企业开关**；`configPath` 由插件配置（`cordis.yml`）显式给出，进程启动时读一次 | 比 Codex 薄，与 Pi/Grok 同档 |
| 审计 | **每次 hook 调用落 session log**：`hook/invoked` / `hook/result` 成对写入（`index.ts:158/182`），还有专门的 invariant 校验配对与 turn 归属（`hook-protocol/src/invariant.ts:31-58`） | 五家里独一份——hook 执行本身是可回放的持久事实 |

**能不能像 Claude Code stop-hook 那样强迫续跑？能，但走的是数据而不是返回值。**

`agent/turn-stopping` 是 serial、没有 `next()`、返回 `void`（`runtime-types.ts:278`），桥的做法是（`hooks-claude-code/src/index.ts:270-277`）：
...
事件声明里那句设计说明是本节最值得抄走的一句（`runtime-types.ts:263-266`）：「a listener that objects steers (`agent.steer(...)`) and the machine re-reads its inbox: fresh steering runs another step, none closes the turn. **Data decides, so listener order cannot change the outcome.**」

第 1 节担心的"多个拦截器顺序导致行为漂移"，dsh 的解法不是定死顺序，而是**取消返回值通道**：所有人只能往 inbox 里放数据，谁先谁后都一样。

代价写在代码里：`// TODO(stop-loop-guard): cap consecutive forced continuations; hooks must self-limit meanwhile.`（`index.ts:269`）——**目前没有强制续跑的次数上限**。

另一处缺口：`continue:false`（强停）被解析和合并了，但 `// TODO(hook-continue-false): merged.stop is logged but needs a run-level halt mechanism.`（`index.ts:189`），即**桥不支持强停**。

### 9.5 两层工具门：可重排的策略层 + 单调的不变量层

第 1 节问过「一个说 allow 一个说 deny 听谁的」。dsh 把这件事拆成两层。

`tools/pre-execute` 是 waterfall，官方叫它 "the reorderable policy layer"（`docs/cookbook/extension-cookbook.md:33`），先注册的监听器（＝外层）可以在 `await next()` 拿到内层结论之后翻盘。

而 `ctx.tools.guard()` 是**注册在 waterfall 之后的单调闸门**：「Any matching guard may deny by returning a reason, while **no guard can force-allow a call another guard denied**.」出处 `packages/core/tools/src/index.ts:1101-1110`。
...
`ctx.effect()` 的语义（`vendor/cordis/src/fiber.ts:402-413`）：effect 体立即执行，产生的 disposer 被收集，**按注册的逆序**在 fiber 卸载或显式 dispose 时执行。`ctx.on()` 本身也是 effect。

全仓 `packages/*/*/src` 下 199 处 `.effect(` / 261 处 `.on(`（收窄到 `ctx.` 前缀则是 165 / 197）——数量级相当，意味着"每个注册都有配对回滚"这条纪律确实落地了。

`llm-retry` 的收尾是个标准范例（`packages/llm/llm-retry/src/index.ts:221-225`）：dispose 时先摘监听器、再 abort 生命周期信号、最后 `await Promise.allSettled([...active])` 排空在途重试。

对中间件生态的实际意义是 `docs/cookbook/extension-cookbook.md:129` 那一行：「Plugin hot-reload | every registration is a `ctx.effect` → vendored HMR just works」——**装/卸一个拦截器不需要重启 agent**，Pi（进程内 import）、Codex（进程启动读 hooks.json）、Grok 都没有这个属性。

### 9.7 事件面：56 个 harness 事件，13 个是拦截点

按 `docs/event-producer-consumer.md`（生成物，`scripts/gen-doc-graphs.ts` 产出）逐行数，主表构成是：
...
| 能否改工具参数 | **不能**：`PreToolDecision` 只有 allow/deny/ask，parsed arguments deep-frozen，注释原文 "Input rewriting is excluded because arguments are already logged and presented"（`tools/src/index.ts:582-587`）；`tools/execute` 的 wrapper 只能换 `exec.signal`；hook 桥同样丢弃 `updatedInput` | 与 Grok 同 |
...
| 异常语义 | `emit` 逐监听器 try/catch + promise catch，"non-vetoing"（`packages/core/agent/src/dispatch.ts:120-136`）；waterfall 异常上抛；hook 桥全线 fail-open | 分档，接近 Pi 的思路 |
...
- ⚠️ 我按 `vendor/cordis/src/events.ts:234-243` 的原样复刻验证了"二次 `next()` 跳过兄弟监听器"，但**没有在 dsh 进程内实跑**（仓库只读、未执行其测试套件）。
- ⚠️ 全仓 376 处 `next()` 我用 grep 做的是模式扫描（`packages` + `vendor` 下所有 `.ts`，含测试），**未逐个函数体核对**是否存在同一 handler 内两次调用。结论"无人靠双 `next()` 重试"是抽查结论。
- ⚠️ `docs/event-producer-consumer.md` 是 `scripts/gen-doc-graphs.ts` 的生成物，我按行统计出 56/41/13/1/1，但**没有独立扫源码验证生成器是否漏收**（例如注释里提到的"deliberately bypass `ctx.emit`"的 contained dispatch）。
- ⚠️ `agent/request-error` 的另一个监听者 `compaction-basic`（`packages/compaction/compaction-basic/src/index.ts:179`）与 `llm-retry` 同时在链上时的裁决顺序、以及 `always` 策略里"先 `await settleDownstream(next)` 再自己兜底"的语义，我只读了 `llm-retry` 一侧，**未追 compaction 侧的完整交互**。
- ⚠️ hook 桥的 `configPath` 是"进程级、加载时读一次"，源码里有 `TODO(per-session-hook-config)`（`hooks-claude-code/src/index.ts:48-52`）；项目级 `hooks.json` 目前是否有其它发现路径，未确认。
