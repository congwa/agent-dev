...
溢出恢复这条路会先把失败的那条 assistant 消息从 agent 内存状态里摘掉，压缩完再重试一次；`_overflowRecoveryAttempted` 保证只重试一次，避免死循环。摘掉失败消息的代码在 `agent-session.ts:2017-2020`。
...
关键在倒数第四行：**找到预算耗尽的位置 `i` 之后，是再往后（更新的方向）找最近的合法切点**——宁可少留一点，也不能切在非法位置。实现在 `compaction.ts:415`：
...
也就是说，上次幸存下来的那批消息，这次会被**重新纳入摘要范围**——它们的内容会进入新摘要，然后才被丢弃。文档里的原话：*"This preserves messages that survived the earlier compaction by including them in the next summarization pass as well."*（`docs/compaction.md:79`）。
...
**摘要模板**（compaction.ts:467-498）是固定六节：`## Goal` / `## Constraints & Preferences` / `## Progress`（下分 Done / In Progress / Blocked）/ `## Key Decisions` / `## Next Steps` / `## Critical Context`，末尾要求 *"Preserve exact file paths, function names, and error messages."*。
...
这两个列表**不依赖 LLM 生成**，而是从被摘要消息里的 `read` / `write` / `edit` 工具调用参数直接抽出来（`utils.ts:29-56`），并且从上一次压缩的 `details` 里累加继承（`compaction.ts:42-70`）。
...
也就是说，**Codex 的压缩后历史 = 近 2 万 token 的原始用户指令 + 一条摘要**。实现在 `build_compacted_history_with_limit`（compact.rs:629），调用方长这样：
...
所以 `insert_initial_context_before_last_real_user_or_summary`（compact.rs:559）要费一大段逻辑去找「最后一条真实 user 消息」的位置，只为了保证摘要仍然是最后一项。
...
返回的 history 不能直接信任，要过一遍白名单（`compact_remote.rs:341`）：
...
V2 路径（`compact_remote_v2.rs:444`）在此之上还有一个 `RETAINED_MESSAGE_TOKEN_BUDGET = 64_000` 的保留预算和 `MAX_RETAINED_AGENT_MESSAGE_TOKENS = 10_000` 的单条上限。
...
一句话：**Pi 是「切点往新的方向挪，多总结一点」；LangChain 默认是「切点往旧的方向挪，多保留一点」**，只有在找不到配对 AIMessage 的异常情况下才反过来向前跳过这些孤儿 ToolMessage。源码在 `langchain/agents/middleware/summarization.py:779`：
...
`recent_messages: extract_messages_since_last_real_user(conversation)`——**最后一条真实 user 消息（排除合成前缀和 `__auto_continue__` 哨兵）之后的全部消息原样保留**，之前的整体换成 LLM 摘要。这行在 `xai-chat-state/src/compaction_utils.rs:581-582`。

压缩后历史结构有注释给出的公式（`xai-grok-compaction/src/code_compaction/assemble.rs:62-90`）：
...
库里另有一套按 token 预算回溯、"向新方向 snap"的 `select_turns_to_compact`（select.rs:29-64，注释同样点名 orphan tool results 会被 API 以 400 拒绝），但那是 Grok **chat** 侧的 intra 压缩用的，不是 grok-build 主路径。
...
| 项 | 值 |
|---|---|
| 默认阈值常量 | `DEFAULT_AUTO_COMPACT_THRESHOLD_PERCENT: u8 = 85`（config.rs:13） |
| 解析五级（依次回落） | env `GROK_AUTO_COMPACT_THRESHOLD_PERCENT` → 用户 per-model → 用户全局 → 远端 per-model → 远端全局 → 85 |
| 另有四类触发 | 手动 `/compact [text]`、context-length 错误恢复、preflight overflow、切小窗口模型（session/compaction.rs:1-8） |
...
分割比例常量是 `TWO_PASS_DEFAULT_SPLIT_FRACTION = 0.95`，指纹字段是 `fingerprint_prefix`，实现在 `session/two_pass.rs:1-13`。
...
压缩后的 `<system-reminder>` 还会重建**活动状态**：Running Background Tasks / TODO List / Running Subagents 三节（reminder.rs:116-211）——压缩不丢后台任务和 todo，这是另外三家都没显式处理的。
...
对应源码 `packages/compaction/compaction-basic/src/region.ts:98-134`，回退循环那几行是：
...
`keepFromIdx -= 1` = **保留区往旧的方向长**，和 LangChain 同向、和 Pi 反向。但压缩区间的 `start` 永远是 `surfaceNodes[0]`（region.ts:130-133），即**每次都从会话最头上压到保留尾之前**。
...
2. 合法性判据不是「消息类型白名单」而是**括号匹配**：`assistant/message` 里每个 `tool-call` block +1、每个 `tool/result` −1，某个 cut 合法 ⟺ 该处未闭合调用数为 0（`packages/compaction/compaction/src/tool-pairing.ts:29-37, 62-68`），并按 `surface.replaceGeneration` 做增量缓存。这是本章「合法切点必须显式枚举」的第五种实现——枚举变成了计数。
3. `compactRegion(start, end, …)` 是公开 API：**任意区间**都能压，两端各查 `toolPairingBalancedBefore/After`，不平衡直接抛错（region.ts:327-333）。文档明说区间「preserve tool-call/result pairing but not whole turns」（`docs/subsystems/compaction.md:86`，非代码注释）——**允许压掉一个超大 turn 的前半段**，正是本章 1.2 说的那种情况，而且不需要 Pi 的两段式摘要。
...
`SurfaceOp` 有且只有两种，`'append'` 和 `{ op:'replace', start, end }`，后者「Used by compaction; any surface-replacing producer may use it」（`packages/core/session/src/types.ts:365-374`）。

一次成功压缩**追加 4 条事件**：`compaction/start` → `compaction/summary`（log-only，不进 surface）→ 一条带 `surfaceOp:{op:'replace'}` 的 `user/message` → `compaction/end`（region.ts:189、447-465、215）。被遮蔽的节点全部列进新节点的 `sourceEventSeqs`，日志里一个字都没删。
...
锁的语义值得抄：`compaction/end` 最后写，所以**中途崩溃留下的是可检测的孤儿 start，而不是一条谎称压缩完成的 end**（compaction.md:19；`compaction/src/invariant.ts:214-216` 断言「successful `compaction/end` requires one `compaction/summary`」）。

失败的尝试也照样落日志、照样闭合，`ManualCompactionErrorCode` 有六种：`busy|cancelled|changed|summary|commit|persistence`（`compaction/src/index.ts:28-35` 与 `command-compact/src/index.ts:23-55`）。
...
`buildSummarizationInput()` 的注释同样点名 *"the call is a genuine prefix of the conversation and reuses the provider's KV cache"*（region.ts:488-494）。
...
代价是每次压缩都要把整段被压区间原样重放一遍当输入（上限约 threshold 减保留尾，不随会话总长增长——区间头就是上一次的 checkpoint），且请求里带着全套 tools（summarizer.ts:158），所以 prompt 里必须写「do not call any tool」——和 Grok 8.3 撞上的是同一个坑。

这次调用还带 `purpose:'compaction'`，llm-deepseek 适配器据此加一个 `x-deepseek-harness-compact: '1'` 请求头（adapter.ts:292-294）。
...
`COMPACTION_INSTRUCTION`（summarizer.ts:31-66）要求 `## Primary Request and Intent / Key Technical Concepts / Files and Code / Errors and Fixes / Pending Jobs / Current Work / Next Step / Critical Context` **八节一节不许少**（"Write \"(none)\" for an empty section — never drop a section."）。

第五家继续印证本章 1.3：所有人都收敛到固定小节模板。

三句逐字值得记：

- 链式压缩：*"If the conversation already contains a `<compacted-summary>` block, it is a PRIOR checkpoint. Do not copy it forward verbatim: preserve still-true facts, drop stale ones, and merge newer information into a single consolidated summary under the same structure."*（summarizer.ts:65）
- 防自指：*"Do NOT mention this summarization request or that the context was compacted."*（summarizer.ts:63）
- 落盘前言（写进那条替换 user 消息，summarizer.ts:69-70）：*"This is an automatically generated checkpoint condensing an earlier span of the conversation to free up context. … Continue the task directly from the messages that follow, without acknowledging this checkpoint."*

三道机械防线，都不靠模型自觉：

| 防线 | 行为 | 出处 |
|---|---|---|
| 摘要含图片 | 直接拒 | summarizer.ts:220-222 |
| `max-tokens` 截断 | 视为失败而非将就（"incomplete checkpoint"） | summarizer.ts:206-209 |
| 框好的摘要 token 数必须严格小于被遮蔽内容 | 否则抛错、本次压缩作废（surface 不动） | region.ts:373-378 |

最后一条是四家都没有的守恒断言。

### 9.5 两条零 LLM 路径：pruner 在压缩前，spill 在进上下文前

- **`ctx.toolResultPruner`**（`compaction-tool-result-pruner`）：确定性 head/middle/tail 剪枝，默认 `thresholdChars 8192 / headChars 4096 / tailChars 1024`（`config.ts:10-14`），中段换成 `'\n\n[... tool result middle pruned ...]\n\n'`（config.ts:7），按 **Unicode code point** 切以免劈开代理对。压力和 overflow 两条路都**先剪再选区间再重新计量**（`compaction-basic/src/index.ts:283-291、308-311`），剪完若已降到阈值以下就**根本不叫模型**。和 LangChain 的 `ContextEditingMiddleware` 相比：那个改的是 deepcopy、state 不动；dsh 是**落进日志的 `replace` 事件**（pruner `index.ts:167-173`），原事件仍在日志里被 `sourceEventSeqs` 引着——持久且可回放。
- **`packages/spill`**（3 个包 / 664 行）：`tools/post-execute` 上把超过 `maxInlineBytes`（base bundle 配 50000）的纯文本结果**整份存盘**，模型侧只留 head/tail 预览 + locator + retrievalHint（`spill-policy/src/index.ts:190-209`），模型可以自己 `read`/`grep` 取回（提示词在 `spill-local/src/index.ts:60`）。这是 Grok「压缩残档可自助找回」的前置版：**大结果压根不进上下文**，从源头减少压缩频次。best-effort：存盘失败就保留原文，绝不把成功的工具调用变成 `isError`（同文件 154-161）。

### 9.6 token 计量：anchor + 有符号 delta，能表示「变小了」

`ctx.tokenMeter` 固定启发式 `chars/4` + 每块 4 token 结构开销 + 每条消息 4 token role 开销（`packages/llm/token-meter/src/estimate.ts:13-19`）。

真正的差异在基线：以最近一次成功请求的 provider usage 为锚，**只在「请求信封完全一致」且「provider 报的总数不低于同口径启发式价」时才采信 usage**（`token-meter/src/index.ts:125-137、246-248`），此后用**有符号** `surfaceDeltaTokens` 跟踪 surface 的增减。

Pi/LangChain 的「usage + 尾巴估算」只能加不能减，压缩后要等下一次真实 usage 才知道降了多少；dsh 因为每次 `replace` 前都紧挨着写一条报价事件（`compaction/summary` 或 `compaction/prune` 的 `shadowedTokenCount`），压缩当场就能把占用算下去（`usage-projection.ts:142-162、203` 的 `projectedTokens`，注释原话是让占用「answers for the next request rather than the last one」）。

阈值、保留量与重试都有校验，摆成表：

| 项 | 值 | 出处 |
|---|---|---|
| `thresholdRatio` / `retainRatio` | 0.8 / 0.16，按模型 contextWindow 缩放 | `compaction-basic/src/config.ts:20、23` |
| 互校验（加载期） | `retainRatio >= thresholdRatio` 在插件加载期就报错 | config.ts:185-190 |
| 互校验（缩放后） | 还有一道 `retainTokens >= thresholdTokens` | config.ts:148-154 |
| `compactionRetries` | 默认 1（最多压两轮），两轮后仍超阈值就抛 | — |
| 异常吞掉 | `agent/pre-step` 监听器吞掉异常继续本轮 | index.ts:155-163 |
| 兜底 | `agent/request-error` 上的 overflow 恢复，`maxOverflowRetries` 默认 1；恢复路径 `retainTokens=0`，只保留最后一个 surface 节点 | index.ts:179-223 |
...
dsh 里 system prompt 根本不在 surface 上（它在 `request/header` 事件里，而 `SurfaceEventType` 只有 user/assistant/tool 三种，`core/session/src/types.ts:200-210、343-346`），每次请求重新渲染，压缩碰不到。

而目录级 AGENTS.md 是以 `user/message` 形式注入 surface 的，`visibleBaselineSource()` **按 `session.surface.nodes` 判定可见**（`packages/context/agent-instructions/src/index.ts:52-57`、`state.ts:140-148`；另加一批尚未落盘的 authority 消息），一旦被压缩遮蔽就视作「不存在」，下一次 pre-step 自动重新组装并注入。

README 的说法是 *"Compaction re-arms a scope after its context event leaves the visible surface even when the cached version is unchanged."*（`packages/context/agent-instructions/README.md:53`，已在代码中逐行确认）。

**声明式对账 vs 结构化重建**，这是第五种答案，而且注入方完全不知道「压缩」这回事。

### 9.8 压缩是不是 capability seam：是，配置级可换

`ctx.compaction` 是标准三角：Service Definition `dsh-compaction`（抽象类 `CompactionEngine`）/ Provider `dsh-compaction-basic` / 人类 Consumer `dsh-command-compact`，见 `docs/subsystems/compaction.md:5`。

换后端只是改一行 bundle 配置：`packages/bundle/base/cordis.patch.yml:284-285` 挂 `compaction-basic`，`:360-365` 挂 pruner，`:346-352` 挂 spill。

子类只需覆盖 `summarize()` 这一个钩子（`compaction-basic/src/index.ts:236-246`，注释：*"Override this sole hook for a template or remote summarizer"*）——Codex 的 remote compaction 在 dsh 是一个子类，不是一条分支。
...
注意两处产品取舍与另外四家都不同：`/compact` **不接受任何参数**（`command-compact/src/index.ts:13、62-63`），没有 Pi/Grok 的聚焦指令；且手动压缩**要求 agent 空闲**，有开着的 turn 直接返回 `busy`（region.ts:172-175）。

### 9.9 本节未确认

- ⚠️ 文档 `docs/subsystems/compaction.md:5` 把 `dsh-command-compact` 写成 seam 的 Consumer，而 `docs/capability-seams.md:457` 的 Consumer 列填的是 `compaction-basic`、并注明「there is no model-facing compact tool」。两处口径不一致，未影响代码行为。
- ⚠️ 头锚定 + 全套 tool schemas 的摘要请求，成本全靠 KV cache 抵消。仓库里没有找到对「第 N 次压缩的摘要请求实际 cache 命中率 / 花费」的任何度量或回归测试，这条设计的收益未被验证。
- ⚠️ `compaction-basic/tests/compaction-loop-repro.spec.ts:20-25` 提到编号事故 `CBR-001`（替换 checkpoint 高 seq 落在 surface 头部导致的回归），但 `docs/postmortem/` 只有 0001–0004、无对应条目，全仓库仅该测试文件出现过该编号（第 21、217 行），事故原始记录未公开。
