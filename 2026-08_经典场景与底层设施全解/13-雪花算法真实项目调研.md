...
三个项目各自盯上了雪花的一个不同软肋，解法互不相同，并排看会更清楚：
...
⚠️ 最后一步的前提是你**有**这张映射表：方案 1、5 天然有；方案 3、4 的映射表就存在 ZK/DB 里；**方案 2 没有**——号是算出来的，事后你根本不知道 7 号是谁。这是 IP/MAC 推导法常被忽略的隐性代价：**可观测性差**。
...
一个规律：真实项目里「**哈希一个本机唯一标识再取低位**」是最主流的做法——本质是**用碰撞概率换零依赖**，赌 1024 个槽位里不会撞。机器少（< 50 台）时这个赌注很划算；机器多或频繁扩缩容就该上中心化分配。Zitadel 的三级降级是最稳妥的折中：先赌，赌不了再问别人。
