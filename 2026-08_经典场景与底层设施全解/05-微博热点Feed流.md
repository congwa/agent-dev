...
> 难度：★★★★☆
> 读完你会懂：
> 1. 推模式（写扩散）和拉模式（读扩散）到底是什么，各自会在什么数量级下崩掉 —— 有具体数字
> 2. 推拉结合的完整判定规则：阈值是**算出来**的，不是拍脑袋定的
> 3. 为什么收件箱里只能存帖子 ID，存内容会死得非常惨
> 4. 多路有序列表怎么合并取 Top N（最小堆），以及为什么绝对不能"全取出来再排序"
> 5. Feed 的"最终一致"为什么用户根本感知不到 —— 这是这个场景最大的红利
...
**"Feed 流"是什么？** 就是你打开微博/Twitter/朋友圈看到的那个**按时间往下滚的列表**。Feed 的英文原意是"喂食"，意思是系统一勺一勺喂给你内容。它的核心特征是：**内容来自你关注的很多个人，混在一起，按某种顺序排好。**
...
**最后两行是 Feed 最大的红利。** 秒杀里你必须用 Lua 脚本、分布式锁、对账系统，是因为错一分钱都不行。Feed 里你有大量的腾挪空间 —— 这也是为什么 Feed 的解法看起来"糙"，但生产上就是这么干的。
...
**这个推论是错的。** 因为写会**放大**。我们把"请求次数"换算成"实际数据库操作次数"：
...
按照惯例，我们先让你把最笨的办法想一遍，然后一条条打破。

**笨办法 1：每次刷新，现查数据库。**

"我不搞什么收件箱发件箱，用户刷新的时候，我直接 SQL 查一下他关注的人最近发了啥，不就完了？"
...
→ 见第 3 节。你会发现明星发一条帖，系统要卡 17 分钟。

**笨办法 3：全用拉，谁都不推。**

"写的时候什么都不干，读的时候现算，多干净。"

→ 见第 4 节。你会发现一次刷新要发 500 次网络请求。

**笨办法 4：加机器。**

"推模式写不过来？加 1000 台 Redis 不就行了？"

→ 部分正确，但你会发现钱不是这么烧的。而且有些问题（比如"改一条帖要改 1 亿份"）加机器解决不了。
...
假设按 `author_id` 哈希分 1000 个库。现在问题来了：
...
"乱序到达"这一条特别关键。推送是**异步**的（后面第 11 节讲），任务可能乱序执行 —— 一条 10:00 的帖子可能比 10:01 的帖子晚到达收件箱。用 List 的话，用户的时间线就乱了。用 ZSet 就无所谓，score 会把它放回正确位置。
...
第二个数字是整个方案的命门。如果一个人可能关注 10 万个大 V，推拉结合就不成立了。但现实中不会 —— **人只能消费有限的内容，这是生理限制，不是技术限制。** 工程上还可以给它加一道保险：限制单用户关注上限（微博是 2000 人），并对"关注的大 V 数"单独设上限（超过就把最不常看的降级成推）。
...
多加的那个"日发帖数 > 100"的分支，就是把第 7.2 节算出来的**摊销系数**用上了。典型场景：某个营销号只有 3 万粉丝（够不上大 V 阈值），但一天发 500 条广告 —— 如果走推，每天要写 3 万 × 500 = 1500 万次，比一个百万粉大 V 发一条还贵。
...
这是这套方案的"另一半"，必须讲清楚，否则用户上线会看到一片空白。
...
**注意第 1 步的设计**：不能让用户等着重建完成再看到内容。做法是"先用拉模式给他一个能看的结果，后台慢慢重建"。这和秒杀里的**降级**是同一个思路 —— 主路径不可用时，用一个更慢但可用的兜底路径。
...
用户刷新时，我们手里有 13 路数据（1 路收件箱 + 12 路大 V 发件箱），**每一路自己都是按时间倒序排好的**：
...
于是我们只需要维护一个大小为 13 的容器，装着"每一路当前的第一条"，反复取出最大的那个：
...
**Python 的 `heapq` 是最小堆。** 但我们要的是"最新的"（score 最大的）。怎么办？
...
**为什么三元组里要有"源编号"？** 除了当书签，还有一个副作用：当两条帖 score 完全一样时（同一毫秒发的），Python 会去比第二项。如果没有它，就会去比 `FeedItem` 对象，而 dataclass 默认不支持 `<` 比较，直接报错。**这是个很常见的坑。**
...
最后那个注意事项是真实的生产坑：**Redis 的 score 是浮点数，超过 2^53（约 9007 万亿）就会丢精度。** 毫秒时间戳现在约 1.7 万亿，安全；但如果你直接把 19 位的雪花 ID 塞进 score，它会被舍入，两条不同的帖可能变成同一个 score。
...
前面 12 节，我们默认 Feed 是**按时间倒序**的。这是最早的做法（Twitter 2006、微博 2009），现在只有少数产品还这么做。
...
推荐系统的召回、粗排、精排、重排、特征工程、模型训练，那是另一个巨大的话题 —— **留给推荐系统那篇讲。** 这里你只需要记住一件事：**Feed 的存储层和排序层必须解耦，靠"时间序召回 + 读时排序"这个接口。**
...
**最关键的是那行带星号的**：读的时候用**当前的关注列表**校验一遍作者。这一行代码，解决了"取关之后收件箱里还有他的旧帖"这个问题 —— **不需要在取关时去清理收件箱**（那是 O(1000) 的写），只需要在读时过滤（O(20) 的判断）。
...
**"宁可重复，不可缺失"是所有异步系统切换状态时的通用原则。** 因为重复是**可检测、可修复**的（去重），缺失是**不可检测**的 —— 用户不会知道自己少看了什么，你的监控也发现不了。
