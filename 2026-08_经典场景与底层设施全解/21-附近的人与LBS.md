...
于是纬度那个范围条件能用上索引，它在这条线上定位出一段连续区间。

但这段区间里的经度是乱的：每一个不同的纬度值下面，都各自有一套经度的排序。第二个范围条件用不上索引，只能把整段区间逐行读出来过滤。
...
用 2.6 节的九宫格函数，代入外滩格 `wtw3sx`，算出它的真实九宫格：
...
- 没有 `GEODEL` 命令。删除成员直接用 `ZREM`，因为它就是 zset，zset 的所有命令都能用。
...
GEO 的编码恰好 52 bit——`2⁵² = 4503599627370496`，塞进 double 一位不丢（随文代码验证过：转成浮点数再转回来，值分毫不差）。
...
搜索半径不够就 k 加一，每圈的边际覆盖是均匀的圆环，不像方格那样越扩越"方"。H3 的 `grid_disk` 函数传入中心格和圈数 k，一个调用就把"中心格 + 邻格"这件事干完，等价于 GeoHash 世界的九宫格，但盖出来的形状接近圆。
...
- **动态点的更新与下线**：人在动，`GEOADD` 对同一成员就是覆盖更新（zset 语义）；用户下线 `ZREM shops:sh beijing`。zset 没有按成员的 TTL，"僵尸位置"要自己清，常见做法是另拆一个 zset，专门存每个成员最后一次上报的时间戳，定时把超时的从两边一起删掉。
