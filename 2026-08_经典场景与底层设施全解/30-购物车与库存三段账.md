...
先看占用那一半，`StockQuerySet.annotate_available_quantity()`（源码，2026-08-05 读取）：
...
**第一半：它被写，而且写得很勤。** `saleor/warehouse/management.py` 里，每次分配和释放都会把它同步更新（当日读取源码）：
...
Saleor 的做法在 `saleor/warehouse/reservations.py`，函数叫 `reserve_stocks_and_preorders()`（当日读取源码）：
...
`management.py` 里算"这个 SKU 现在还剩多少能卖"用的就是它（当日读取源码）：
...
Saleor 在 `Reservation` 上建了 `models.Index(fields=["checkout_line", "reserved_until"])`（当日读取源码），但真正的兜底还是定期删除。**结论没变，只是从"必须删"降级成了"应该删"。**
...
vendurehq/vendure（8,296 star，pushed 2026-08-05，TypeScript，2026-08-05 实测）不这么干。它在 `packages/core/src/config/order/stock-allocation-strategy.ts` 里把这个选择做成了一个可替换的策略接口（当日读取源码，全文只有 32 行）：
...
关于 yudao 的购物车实现：它的 `yudao-module-trade/.../service/cart/CartServiceImpl.java` 是本篇标本之一（路径来自选题调研，2026-08-05 核实到文件级），但**它是否把购物车落 Redis，本次未核实**，以官方仓库为准。
