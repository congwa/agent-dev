...
> **Lilishop 的秒杀，是一条「平台排档期 → 商家自助报名（无审核）→ MQ 异步写 ES 上架 → 前台按时段展示（无缓存）→ 加购结算只做软校验 → 下单不扣库存 → 支付成功后由 consumer 用一段 Redis Lua 脚本原子扣减、失败即整体回滚并自动取消退款」的完整促销流水线；它靠「Redis 单线程 + Lua 原子脚本」把超卖堵死在最后一关，代价是把「抢购的残酷」推迟到了付款那一刻，而在限购、限流、防黄牛、缓存预热这些环节上基本是空白 —— 它是一份非常适合用来读懂"电商促销体系全貌"的教材，但要扛真正的大促流量，前台缓存和扣库存时机这两处必须先改。**
