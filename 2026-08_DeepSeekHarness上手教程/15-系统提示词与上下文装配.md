...
判据很好记：会随会话状态反复变的事实（沙箱模式、审批策略）用 `context`；一次装好基本不动的规矩用 `section`。
...
`PromptAssembly` 就是 `{ sections, contexts, tools, variables }`（`:115`–`:120`），**四样都可以增删改**，返回值即权威——事件文档 `:20`–`:26` 明写 "The returned value is authoritative"。唯一的例外是第 ⑥ 步那两条收尾约束。
