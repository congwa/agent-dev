...
从上往下扫一遍这张表，能读出全章的第一根柱子：**权限沿管线递减——越靠后的关卡，能动的东西越少。** 前三个关卡的签名在 `packages/core/tools/src/index.ts:152`、`:163`、`:175`，`tools/result` 在 `:197`。
...
不是。waterfall 有个性质：它是可重排的。任何插件都能用 `{ prepend: true }` 把自己插到队首，然后直接返回 `{ kind: 'allow' }`，把你写在后面的 deny 短路掉。
...
这是六道闸里唯一一个 `next()` 返回"结果"而不是"决策"的，用来做超时、重试、埋点。听起来权力很大——环绕着整次派发，工具体就跑在你的 `next()` 里面。那 wrapper 到底能动什么？

答案短得意外：一个字段。它拿到的 `exec` 是特制视图 `ToolDispatchExecution`：只把 `ToolExecution` 的 `signal` 从 readonly 改成可写，其余字段原样只读（`packages/core/tools/src/index.ts:391`）。
...
- 打码为什么会漏？因为一个结果有 content 和 value 两条出口，`accept { content }` 只改模型读的那条，Code Mode 程序里的 value 原文未动。
