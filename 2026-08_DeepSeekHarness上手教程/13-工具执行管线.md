...
**多个 wrapper 按注册顺序套娃，先注册的在外层。** 这点在 cordis 源码里能直接读出来：`waterfall()` 拿到监听器数组后从**头部** shift，一个个往里递（`vendor/cordis/src/events.ts:234`–`:243`），入列顺序由 `register()` 的 `push` / `unshift` 决定（`:255`），于是 `{ prepend: true }` 等于抢最外层。内置超时插件的 README 给了这件事的业务含义：超时注册在外层 = 超时覆盖整个重试过程，注册在内层 = 超时覆盖每次尝试（`packages/guard/timeout-policy/README.md:36`）。
