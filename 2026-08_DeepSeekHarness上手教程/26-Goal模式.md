...
驱动器里这个方法有 12 个调用点，最典型的三处是：加载到已有 agent 上、持久化刷盘失败时、卸载前；agent 报错、驱动任务自身抛错、turn 结束时报超 token，同样会先收枪[^11]。
...
下面每一行左边是直觉，右边是实际，出处按行给在脚注里。

| 你可能以为 | 实际 |
|---|---|
| 卸载过程中在跑的那一轮不算数 | Cordis 卸载是异步的，已经被 inbox 接受的那一轮可能开跑并**真的扣掉**轮次号；teardown 只会取消请求、disarm、等待静默，不会假装那次 admission 没发生过[^52] |
| provider 抖动、持久化失败会自动重试 | 一律不自动重试，要人后续授权 resume；源码里对应的动作只有两条：turn 结束报超 token 直接收枪，中止则视 attempt 状态标取消或收枪[^53] |
| 一个会话可以并行挂多个目标 | 只有一个当前目标，没有独立的 goal 数据库；替换或 clear 之后历史仍在日志里[^54] |
| `complete` / `blocked` 有独立校验 | 记录的那个调用者就是权威，模型说完成就是完成[^54] |
| 变更事件的版本会跟着演进 | 版本号钉死在 1，没有兼容承诺，也没有迁移路径[^55] |
| 工具被某个 scope 藏起来，那段 goal 策略提示也跟着消失 | 提示词注册与工具过滤是两件事，工具可能被藏了，策略提示仍然留在系统提示词里[^56] |
| headless / ACP / JSON-RPC 也能用 `/goal` | 这些适配器不消费命令服务，`/goal` 在那些形态下不可用[^57] |
