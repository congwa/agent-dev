...
关于 harness，最流行的理解写在 dsh 官方中文 README 里：它把这个词保留原文，然后加了个括号注解——"由 DeepSeek AI 开发的开源 agent harness（智能体框架）"（`README.zh.md:5`）。
...
所以工程量不在那行 `while (await this.turn()) {}` 上，而在它周围那七十个包里。
