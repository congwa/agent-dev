...
几乎所有人对 agent 内部的想象是一个 `messages` 数组：用户说一句 push 一条，模型答一句 push 一条，压缩就是 splice 掉中间一段，分叉就是 slice 一份副本。数组就是状态本身。这么写没错，能跑，绝大多数项目都这么干。
...
两种写法的差别不在性能，在于**能不能回头**。会计不擦账：写错了不许用橡皮，只许再记一笔冲销。dsh 的日志就是这种账本——数组被 splice 掉的那段就没了，日志里的事件则一个字节都不会少，压缩只是往后追加一条"从这里到这里，改用这段摘要显示"。从账本这个比喻能直接推出一个结论：三天后你想知道当时到底给模型看了什么，答案是可计算的，不是"如果当时打了日志的话"。
...
那日志本体存哪？答案可能出乎意料：它也只活在内存里。存不存硬盘是插件的事——模块注释直接说了 persistence is a plugin concern，插件订阅 `session/event`，在 `session/flush` 时刷盘（`session/src/index.ts:3`）。连"要不要落盘"都被拆成了可插拔的一层，这很符合这个项目的一贯作风。
...
对外确实有三个方法，但它们只是同一个 `send()` 的三种参数组合（`packages/core/agent-loop/src/agent.ts:113`）：
...
门口还有一道闸，容易被忽略，但踩上了很难查。如果当前这一轮已经被取消（用户点了停止），这时候来一条"叫醒型"的输入，它不会挤进那个正在死掉的轮次，而是被自动改判去 `next-turn`。源码注释解释了原因：唤醒型输入不能加入一个已中止的活动。而且这个判断在插入之前就做完了，防止插入过程中触发的回调把分类又改回去。改判逻辑见 `:115`–`:118`。
...
agent 什么时候继续、什么时候停下来？很多实现的答案是一个状态机，或者一个 `shouldContinue` 标志。你可能以为 dsh 这种讲究的项目会有更精密的版本。
...
内层是一轮之内的步进。这里有个乍看很怪的写法：退出判据写了两遍，中间夹着一次询问（`:295`–`:299`）：
