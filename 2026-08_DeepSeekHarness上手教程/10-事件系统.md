...
Cordis 的事件总线只有 352 行，`vendor/cordis/src/events.ts` 一个文件装完，五种派发模式。听起来没什么可讲的。

直到你发现选错模式的后果不是"写得不够好看"，而是**你想要的那个能力，那个扩展点根本不给你**。想拦下一条危险命令，结果挂在了一个只能围观的事件上——代码写完，跑起来不报任何错，命令照常执行。

所以这一章的重点不是背 API，是学会看一眼事件签名就判断出三件事之一：这里我能观察、能改结果，还是能一票否决。
...
declaration merging 指 TypeScript 允许你在别处再打开同一个 `interface` 往里加成员，两处声明合并成一个。事件名和签名就靠合并 `interface Events` 来声明，`ctx.emit` / `ctx.on` 才有类型：
...
完整教程示例在 `docs/cordis-tutorial/04-events.md:11-42`。
