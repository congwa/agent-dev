...
目标窄得有点故意：**把一个你自己写的文件挂进正在跑的 dsh，并且能证明它真的被加载了。** 就这一件事。Service 怎么对外提供（第 07 章）、effect 与卸载（第 08 章）、`Config` schema 校验（第 09 章）都不在这章。
...
这里有个不对称，第一次写很容易踩：命令行上的 `~` 和 YAML 里的 `~` 命运完全不同。
...
真正值得记一笔的是：上面第一节那个 `hello.ts` 走的是**哪一条**。
