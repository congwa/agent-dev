...
有个地方第一眼容易读错：配置树 id 是 `bash-sandbox`，但它注册的服务名是 `ctx.shell`。

也就是说，这一行**就是**默认组合里 `ctx.shell` 的实现，不存在「先挂 bash-local 再叠沙箱」的写法。README 原文说得很直白：“Load it **instead of** `@deepseek-ai/dsh-bash-local`”。
...
这三个依赖在 base 里各有提供者：`subprocess` 来自 [subprocess-local](./dsh-subprocess-local.md)，`sandbox` 来自 `@deepseek-ai/dsh-sandbox-local`，`sandboxPolicy` 来自 `@deepseek-ai/dsh-sandbox-policy`。

出处：树上那一行见 `packages/bundle/base/cordis.patch.yml:178-182`；“instead of” 那句见 `README.md:5`；inject 见 `packages/shell/bash-sandbox/src/index.ts:45`；三个提供者依次见 `cordis.patch.yml:163-164`、`169-170`、`172-176`。
...
覆写的方法有五个：`sandboxMode` getter（返回 `ctx.sandboxPolicy.defaultMode`，工具层据此决定要不要公开升权字段）、`resolve()`（把完整 per-call policy 盖到 spec 上）、`run()`、`start()`、`onProcessDone()`。

对应 `src/index.ts:71, 75-77`（getter）、`:84-86`（`resolve()`）。

## 配置项

`export type Config = LocalConfig`——它**没有自己的配置**，逐字继承 `@deepseek-ai/dsh-bash-local` 的：
...
**沙箱模式与 workspace root 不在这里**，它们归 `ctx.sandboxPolicy`。base 里写在 `sandbox-policy` 那一行：`mode: process.env.DSH_PERMISSION_MODE ?? 'workspace-write'`、`workspaceRoot: process.cwd()`。

出处：类型别名见 `src/index.ts:35`；表格字段来源 `packages/shell/bash-local/src/index.ts:105-112`（`DEFAULT_GRACE_MS` 35 行、`DEFAULT_MAX_SPILL_BYTES` 38 行、`graceMs` 上界校验 90-92 行）；policy 归属见 `src/index.ts:29-34`，base 的那一行见 `cordis.patch.yml:172-176`。
...
这张表出自 README（`README.md:11-15`），它把 `read-only` 标为 default——那是包自身文档的口径；**base bundle 实际默认是 `workspace-write`**，见上一节的 `sandbox-policy` 行。

代码里的分岔很浅，`danger-full-access` 是 `run()` / `start()` 的第一个分支，直接走 `super`：
...
对应 `src/index.ts:91-94, 119`。一条命令从进来到落地按模式分岔：
...
受限模式下无可用 runner 时抛 `SANDBOX_UNAVAILABLE`。

出处：三条文本见 `README.md:59`，前两条的构造函数在 `packages/sandbox/sandbox/src/escalation.ts:71-73, 84-86`；`SANDBOX_UNAVAILABLE` 见 `src/index.ts:103, 111`，错误码常量在 `packages/sandbox/sandbox/src/index.ts:124`。
...
靠签名做可移植推断的代价是双向的：一个恰好匹配签名的应用错误会被误判为拒绝，而被截断、丢掉了那行拒绝信息的输出则会被漏判（`README.md:86`；判据实现在 `src/helpers.ts:67-69, 112-116`）。

**后台 runner 失败没有即时错误通道。** 它记在已结算进程上，等调用方 `job_output` 才浮现；只有同步抛出、且能指认 runner 路径的 `SubprocessRuntime` 错误才会让 `start()` 立即失败（`README.md:87`、`src/index.ts:129-133`）。

**runner 归因写得非常保守**，任何一条不满足就退回普通命令启动失败语义：
...
出处：`packages/shell/bash-sandbox/src/helpers.ts:39-53`、`README.md:20`。

**`danger-full-access` 是刻意绕开 `ctx.sandbox` 的。** 它是「明确的不限制模式」，不是「更宽的沙箱档位」（`README.md:88`）。

## 相关

[tool-bash](./dsh-tool-bash.md) 是它唯一的模型侧消费者，也是升权审批的持有者——本包 deny-only，从不自己谈判权限（`README.md:25`）。
