...
`dshHomePath()` 把段拼到 Harness home 上，home 未由 `$DSH_HOME` 指定时默认 `~/.dsh`，所以默认根目录是 `~/.dsh/sessions`。

`root` 是**必填、无默认**的。这看着有点不近人情，理由却很实在：若默认成 `process.cwd()`，进程 cwd 一变（bash 调用、子进程）会话文件就散了。

出处：bundle 行见 `packages/bundle/base/cordis.patch.yml:98`；`dshHomePath()` 见 `packages/util/home-paths/src/index.ts:98`，home 默认值见同文件 `:80`、`:89`；`root` 无默认的理由见 `packages/session/session-persistence-jsonl/README.md:26`。
...
四个派发模式的定义见 `docs/event-producer-consumer.md:41`–`44`。

有个容易看漏的归属问题：这些监听器是共享的 `PersistenceCoordinator` 装的，不是本包直接写的。也正因如此，HMR 热重载不会重放 `session/created`，所以加载时会把已存在的活会话补种一遍（`coordinator.ts:1136`）。
...
| `packChunks` | `boolean` | `true`（`src/index.ts:37`、`128`） |
| `compression` | `'zstd' \| 'none'` | `'zstd'`（`src/index.ts:38`、`57`） |
| `preparedSessionCacheSize` | 正整数 | `5`（`packages/session/session-persistence/src/coordinator.ts:27`） |
| `writeBatchMaxDelayMs` | 正整数 | `200`（`coordinator.ts:30`） |
...
它约束的只有这个窗口本身，不约束事件循环、序列化操作或后端本身的延迟。上限是 Node 的 `2_147_483_647` ms（`packages/util/timeout/src/index.ts:25`）。
...
其中 `delegationDepth` 在盘上是必填的，顶层会话为 `0`，缺失或非法直接拒绝整份日志。`agentPreset` 之所以耐久，是因为它决定恢复后会话的工具与 prompt——换一套组合去重放历史，模型会看到自己再也执行不了的动作（`README.md:17`）。

`seq` 在解码后的日志里保持连续，即 `events[i].seq === i`。

路径里那两级目录各有各的脾气。项目目录名是把 cwd 规范化后**有损**截断和替换分隔符得到的，所以规范化后相同的 cwd 会共用一个项目目录；会话 id 仍然区分出各自的会话目录（`README.md:19`）。会话 id 本身是未经校验的 branded string，会被单射转义成一个安全路径段（`README.md:20`）。

耐久语义有五条要点，都来自 `README.md:42`–`48`：

| 语义 | 具体行为 |
|---|---|
| 懒物化 | `create()` 什么都不写，首次 `append` 才写 header 帧并 `fsync` |
| 发布方式 | POSIX 用硬链接（`src/index.ts:549`），Windows 用 `MoveFileExW(..., MOVEFILE_WRITE_THROUGH)`（`src/win32.ts:30`、`:47`），两者均不覆盖 |
...
```
if 最后一帧结构不完整:
    if 损坏点晚于最后一个已提交 turn/end:
        保留该帧已解码的记录
        从该帧起截断
        补上契约要求的合成 tool/step/turn 收尾
    else:                      # 落在最后一个 turn/end 处或之前
        整份日志判定为损坏，直接拒绝，不做任何修复
```
...
JSONL 存储**不贡献任何 live prompt 或 schema**（`README.md:60`）。
...
同 seam 另有一个可互换后端 `@deepseek-ai/dsh-session-persistence-sqlite`（`docs/config-catalog.md:1588`）。它是一行一个 `SessionEvent`，`events` 表列为 `(session_id, seq, type, time, data, source_event_seqs, surface_op)`，与事件 1:1（`packages/session/session-persistence-sqlite/README.md:11`、`src/schema.ts:142`–`143`）。
...
所以正确写法是禁用旧行 + 插入新行（断言行为见 `vendor/include/src/index.ts:116`–`119`，`insert` 语义见同文件 `:80`–`95`）：
...
把持久化整个关掉也合法：`disabled: true`（patch 层删不掉 bundle 行）。会话变成纯内存，代价是所有 inject 了 `sessionPersistence` 的插件都不会激活——[session-checkpoint-policy](./dsh-session-checkpoint-policy.md)（`docs/config-catalog.md:3078`）、[session-projection-cache](./dsh-session-projection-cache.md)（`:1635`）、`dsh-schedule`（`:3076`）。
