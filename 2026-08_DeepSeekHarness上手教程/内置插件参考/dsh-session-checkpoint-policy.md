...
三个位置是：模型 adapter 收到请求之前、顶层工具体可能产生外部副作用之前、每个 `agent/pre-step` 边界上。
...
bundle 给这一行的注释是 `Durability checkpoints before each model request and top-level dispatch.`（`cordis.patch.yml:354`）。

这一行没有 `config`，`inject` 也不在 YAML 里——它写在代码里。
...
| 事件监听 | `llm/stream`（**waterfall**） | 懒包装下游流：先 `await ctx.sessions.flush(session)` 再 `yield* next()`，所以下游流在缓冲的请求事件耐久之前根本不构造 |
| 事件监听 | `tools/execute`（**waterfall**） | 只对**有 agent 且无 parent** 的顶层调用生效。flush 之后若已取消，返回规范的 aborted 结果，**不进工具体** |
| 事件监听 | `agent/pre-step`（**waterfall**） | 每次请求派生之前，把上一 step 提交的响应/结果批次持久化 |
...
`llm/stream` 的特别之处在于"懒"——包装的是生成器，flush 排在 `yield*` 之前，下游那条流在事件落盘之前压根不存在：
...
`tools/execute` 的豁免条件写死在一个判断里，非顶层调用直接放行：
...
出处：`llm/stream` 见 `src/index.ts:64`–`68` 与 `34`–`37`，派发模式见 `docs/event-producer-consumer.md:39`；`tools/execute` 的顶层判定见 `src/index.ts:71`，取消分支见 `src/index.ts:70`–`75` 与 `docs/event-producer-consumer.md:56`；`agent/pre-step` 见 `src/index.ts:79`–`82` 与 `docs/event-producer-consumer.md:18`。
...
无配置项。README 称之为 `zero-config function plugin`（`README.md:9`）。
...
`README.md:19` 说得很直白：持久化和检查点调度**故意是两个 Cordis 插件**。分工是这样的——
...
只加载后端不加载策略是合法组合，但崩溃时可能丢掉还在批窗里或还在写的事件。第一方持久化 app 与 runtime 两个插件都显式挂，特化部署可以故意省略或替换策略。

失败语义（`README.md:23`）分两种，别混：
...
取消时的规范结果由本包物化（`src/index.ts:41`–`50`）：
...
**本插件不加任何 prompt 或工具 schema**（`README.md:31`）。
...
**一、关掉它**（README 明确背书这种组合，`README.md:19`）。bundle 里的行在 patch 层删不掉，只能禁用：
...
这里有个坑：`config` 不能停一行，`disabled` 才能（patch 三种形态见 `packages/boot/app-boot/src/profile.ts:127`–`130`）。

**二、换成自己的策略插件**。它没有 provide 任何服务，只是三个 waterfall 监听器，因此"替换"就是禁用这一行、再 `insert` 一个挂在同样三个事件上按你的规则 flush 的新插件（`insert` 语义见 `vendor/include/src/index.ts:80`–`95`）。

别想着直接改本行的 `name`——那会因 name mismatch 被整条跳过（同文件 `:116`–`119`）。
...
读源码补充一条 README 没写的：`llm/stream` 上的包装只在 `options.sessionId` 存在**且**该会话还在 store 里时才生效，否则直接 `next()`（`src/index.ts:65`–`67`）。

也就是说，非会话绑定的辅助模型请求（例如标题生成）不会被它拦。
