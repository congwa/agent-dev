...
> `@deepseek-ai/dsh-llm-retry` · bundle：`base` · 配置树 id：`llm-retry` · v0.1.0-rc.5（commit `47f9438`）2026-08-14 核对，出处收在文末脚注。
...
它在树上只占这两行，不带 config——它**没有** config[^1]。源码级 `inject = ['agents']`，要的是 `@deepseek-ai/dsh-agent` 提供的 `ctx.agents`，不是 `ctx.llm`[^2]。

## 它注册了什么

| 类型 | 名字 | 说明 |
|---|---|---|
| 事件监听 | `agent/request-error`（**waterfall**） | 它要么返回 `{ kind: 'retry' }` 自己接管，要么 `next()` 交给下游[^3] |
| session 事件类型 | `llm/retry` | **非 surface** 的持久记录，等待**之前**就落盘[^4] |
| session 事件类型 | `llm/retry-started` | 退避真的等完了才写，紧接着才返回 `{ kind: 'retry' }`[^5] |
| 清理 effect | 标签 `llm-retry: abort and drain active recovery` | 摘监听、abort 生命周期、`Promise.allSettled` 排干在飞的恢复[^6] |

不注册服务、工具、prompt 段或命令。

同包另发布的 `./invariant` companion 另有监听——`session/created` 与 `internal/dispatch`——那是校验用的，不参与恢复决策[^7]。
...
`Config` 就是空对象[^9]。`validateConfig` 还专门给最容易犯的那个错留了诊断：往这里写 `retryPolicy` 会得到报错 `llm-retry: retryPolicy belongs under each provider configuration`[^10]。
...
它们在路由注册那一刻被 [llm](./dsh-llm.md) 抓取冻结成 `ResolvedRetryPolicy`，随每次调用传到最终 adapter 边界。冻结这件事有个直接后果：**路由事后被 dispose 或 replace，在飞的那次失败仍然沿用它当时的那份策略**[^11]。
...
`always` 模式没有次数上限——次数检查只写在 normal 分支里[^13]。它先问下游要不要接手，再无限重试**任何**模型请求失败，直到成功、取消或插件被 dispose[^14]。

退避有两条硬规则，核心是 provider 回给你的 `providerRetryAfterMs` 什么时候作数[^15]：
...
README 的 Model Experience 先给了一句总结[^16]：
...
（`disabled` 是 patch 的合法字段，停用的 entry 不会被挂载[^17]。）

此时 `agent/request-error` 的默认行为生效——原样保留失败，失败即终态[^18]。
...
配套的 `./invariant` 子插件会校验一串东西[^19]：每次调度的重试都指向当前打开的 turn 和最近关闭的 step、匹配失败请求的持久 provider、身份非空、按模式有对应边界、step 记录唯一、provider-policy 维度的重试号正确、定时器延迟有界。

它还要求每个 `llm/retry-started` 都能对应上一条同 `retryId`/turn/step/retry 的调度记录，重复的 started 事件直接拒绝。

## 坑与边界

以下是 README 的 Known Limitations and Deferred Work[^20]：
...
读源码补充：重试号的连续性不是靠内存里的计数器，而是靠回查上一条 `llm/retry` 事件，四个维度都得对上[^21]：
...
`policyKey` 把每一个影响行为的字段都编进去，normal 模式还会给 `retryableCodes` **排序**，因为资格判定用的是集合成员关系（顺序不该影响身份）[^22]。所以路由一旦被换成不同的限次/码集/退避，`policyKey` 就变了，重试历史随之**从头开始**。

另外重复一遍那个容易漏的点：退避途中被取消**不会**写 `llm/retry-started`[^23]。

---

## 出处

[^1]: 树上这两行：`packages/bundle/base/cordis.patch.yml:72-73`。
[^2]: `inject`：`packages/llm/llm-retry/src/index.ts:21`；`ctx.agents` 的提供方：`packages/core/agent/src/index.ts:267`。
[^3]: 监听注册在 `src/index.ts:210`；事件声明在 `packages/core/agent/src/runtime-types.ts:260`，`@mode waterfall` 标在 `:258`；另见 `docs/event-producer-consumer.md:20`。
[^4]: 类型声明在 `src/types.ts:6-13`，写入在 `src/index.ts:150`。
[^5]: 写入在 `src/index.ts:152`——退避真的等完了才写，逻辑在 `:151` 的 `cancellableDelay` 返回 true 之后。
[^6]: `ctx.effect(...)`：`src/index.ts:221-225`。
[^7]: `src/invariant.ts:159-160`。
[^8]: `README.md:29`。
[^9]: `src/index.ts:24-27`。
[^10]: `src/index.ts:29-36`。
[^11]: `README.md:7`。
[^12]: `packages/llm/llm/src/retry-policy.ts:14-24`。
[^13]: `src/index.ts:190`。
[^14]: `src/index.ts:161-176`。
[^15]: `README.md:9`、`src/index.ts:194-205`。
[^16]: `README.md:31-45`。
[^17]: `disabled` 是 patch 的合法字段：`vendor/include/src/index.ts:151`；停用的 entry 不 mount：`vendor/loader/src/config/entry.ts:126`。
[^18]: `packages/core/agent/src/runtime-types.ts:245-249`。
[^19]: `README.md:13`。
[^20]: `README.md:47-53`。
[^21]: 回查逻辑：`src/index.ts:182-188`。
[^22]: `policyKey` 构造与排序：`src/index.ts:65-76`。
[^23]: 取消不写 started：`src/index.ts:151`。
