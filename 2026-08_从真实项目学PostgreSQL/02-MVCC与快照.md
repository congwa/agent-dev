...
**❌ 用默认的 READ COMMITTED 会怎样**：5 条 SQL 各取一个快照，中间用户还在充值和消费。你统计"总消费"时算进了一笔单，统计"当前余额"时那笔单又已经扣完了——**账永远对不平，而且每次跑出来的差额都不一样，你会以为是代码有 bug，查三天查不出来**。
...
B 里还额外多了一个 128 kB 的 `balance` 索引。也就是说：

**给一个高频更新的列建索引，会同时付出三笔代价：**
...
旧版本变成死元组之后，需要 VACUUM 回收。VACUUM 做三件事：

1. 把确定没人需要的死元组占用的空间标记为可复用（**注意：是"可复用"，不是"还给操作系统"**）；
2. 清理指向死元组的索引项；
3. **freeze**：把足够老的行版本的 xmin 标记为"永远可见"，防止事务号回卷。

关键在于 VACUUM 怎么判断"没人需要"：它计算一个 **回收水位线（OldestXmin）**——所有正在运行的事务里最老的那个快照。**比这个水位线更新的死元组，一律不敢碰。**
...
如果一个行版本的 xmin 老到超过 21 亿个事务没被处理，它会突然从"很老所以可见"翻转成"来自未来所以不可见"——**数据凭空消失**。
...
- **16 亿代**（PG 14+ 的 `vacuum_failsafe_age`）：进入紧急模式，跳过索引清理、不再限速，一切让路给 freeze；
- **距回卷点还剩 4000 万个事务时**（即年龄约 21.07 亿）：日志开始刷 `WARNING: database "x" must be vacuumed within N transactions`；
- **距回卷点还剩 300 万个事务时**：**数据库拒绝分配新的事务号**（`database is not accepting commands to avoid wraparound data loss`），唯一的出路是停服、单用户模式 `VACUUM FREEZE`，大表可能要跑几小时。
