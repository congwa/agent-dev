...
> 整套文档的主线章节。看完这篇，你应该能对着任何一条"条件更新"SQL 说清楚它在并发下是安全的还是不安全的，以及为什么。源码与实测脚本出处收在文末脚注。

---

## 一、案发现场

[sub2api](https://github.com/Wei-Shaw/sub2api) 是一个 AI API 网关，Go + PostgreSQL 15+，34.4k★，按 token 真金白银地给用户计费。
...
同一个 repo 的另一个方法里，作者把理由直接写在注释里了[^2]：
...
实测（初始余额 1000，20 个线程共扣 100 次，每次扣 1）[^3]：
...
而且这只是 100 次操作的低并发实验。线上几千 QPS 打在同一个大客户账号上，丢失比例只会更高。
...
**根因**：`SELECT` 和 `UPDATE` 之间有一个"时间缝"，缝里发生的事情，你的应用代码一无所知。

`bal-amount` 这个值是在应用内存里算的，它携带的是一个**已经过期的世界观**。
...
if 等锁期间这行被别人改过:
    行 = 取该行的最新已提交版本()     // 不用我进来时那个快照
    if not WHERE条件(行):            // 重算的是 WHERE，只有 WHERE
        放弃这一行                    // RowsAffected 不增加
        return

执行 SET(行)                         // 产生新行版本
return RETURNING(新行版本)
```
...
第 ④ 步是整件事的核心，Postgres 源码里叫 **EvalPlanQual**[^4]。它是 READ COMMITTED 下"写操作看得见最新数据"的实现机制。
...
初始余额 100，T1 并发把它改成 50，T2 想扣 80[^5]。
...
发生了什么：T2 被 T1 的行锁挡住 → T1 提交（余额变成 50）→ T2 拿到锁 → **EPQ 用新版本重算 `balance >= 80`，`50 >= 80` 不成立 → 这一行被放弃 → 更新 0 行**。

应用代码只要检查 `RowsAffected() == 0` 就知道余额不足，**完全不需要自己去锁、去比较、去重试**。

---

## 四、⚠️ 条件必须写在 WHERE 里，写在应用层没用
...
**1. 慢一倍。** 实测（16 并发全打在同一行，共 1600 次扣款）[^6]：
...
三种写法结果都对，但吞吐差 7.4 倍。原因很直接：

| 写法 | 网络往返 | 行锁持有区间 |
|---|---|---|
| A 单条原子 UPDATE | 1 次 | 仅语句执行期间 |
| B SELECT FOR UPDATE + UPDATE | 2 次 + 一次显式 BEGIN/COMMIT | 从 SELECT 一直持有到 COMMIT |
...
我第一遍读也以为这是个漏洞，其实是一个**深思熟虑过的业务权衡**：
...
**但这不影响正确性**——第二条 UPDATE 依然是原子的 `balance = balance - $1`，无论此刻余额是多少，扣的金额都对。

**变的只是"这笔算不算透支"这个分类，不是钱的数目。** 而且这两条语句在同一个 `tx` 里，外层事务保证了它们和写流水、写幂等键是一起提交的。
...
| 写法 | 性质 | 余额不足时 | 你能知道差多少吗 |
|---|---|---|---|
| `GREATEST(balance + $1, 0)` | 钳位 | 扣到 0 为止，语句成功 | 不知道少扣了多少 |
| `WHERE balance + $1 >= 0` | 拒绝 | 0 行，语句"失败" | 知道扣不动 |
...
把这一章的结论串一遍，每一条都能扣回前面某一节的实验或画面：

- 之所以"读-改-写"必翻车，是因为 SELECT 和 UPDATE 之间那道时间缝——应用代码算出的新值，携带的本来就是一个已经过期的世界观（第二节）；
- 之所以一条 UPDATE 就够，是因为 EvalPlanQual 会在拿到行锁之后重新验证 WHERE，读和写被压进了同一条语句、同一把锁（第三节）；
- 之所以约束必须写进 WHERE、写在应用层等于没写，是因为 EPQ 重算的只有 WHERE 子句，你在别处做的判断它一概看不见（第四节）；
- 之所以 REPEATABLE READ 下同样的写法会报错而不是悄悄重试，是因为 RR 的快照承诺不允许 EPQ 偷看最新版本，Postgres 索性让你自己重试（第五节）；
- 之所以 RETURNING 要一次拿全新旧值，是因为多一次 SELECT 就是多留一道缝，别人的扣款会趁机插进来（第六节）；
- 之所以那两条透支 UPDATE 之间的缝隙不影响正确性，是因为第二条本身仍然是原子的，缝隙改变的只是"算不算透支"这个标签，不是钱的数目（第七节）。
...
[^2]: `AdjustBalance` 方法所在文件：`backend/internal/repository/user_repo.go`。
[^3]: 丢失更新实测脚本：`labs/exp1_lost_update.py`。
[^4]: EvalPlanQual 的 PostgreSQL 源码文件：`src/backend/executor/execMain.c`。
[^5]: EPQ 覆盖重算实测脚本：`labs/exp2_epq.py`。
[^6]: `SELECT FOR UPDATE` 与原子 UPDATE 吞吐对比实测脚本：`labs/exp17_perf.py`。
