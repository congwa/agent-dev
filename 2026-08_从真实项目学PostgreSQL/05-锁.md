...
> **一句话结论**：Postgres 里根本没有"一种锁"。表锁住在共享内存的一张定长哈希表里，**行锁压根不在那张表里**——它被写在数据行自己的头部（`t_xmax` + `t_infomask`）。

这一个设计决定，直接解释了后面所有让人困惑的现象：为什么锁一百万行不费内存、为什么 Postgres 没有"锁升级"、为什么 `pg_locks` 里永远查不到行锁、为什么等锁的人显示在等一个叫 `transactionid` 的东西、以及为什么 `ALTER TABLE` 能让整张表停摆。
...
Postgres 内部有**四类互不相干的锁机制**，它们的实现、成本、用途完全不同，只是中文里都叫"锁"。源码 `src/backend/storage/lmgr/README` 开头就在讲这件事。
...
想加死锁检测，就得维护"谁在等谁"的等待图 → 需要共享内存里的数据结构 → 而维护那个结构本身又需要加锁。所以最底层必须有一种"不需要任何登记、纯靠 CPU 原子指令"的锁 —— 这就是 spinlock。
...
第一，别指望"我算好了公式就不会炸"——它可能在你以为安全的地方炸，也可能在你以为会炸的地方撑住。留足余量。

第二，报这个错时，真正该查的是**"谁在一个事务里摸了几百上千个对象"**。头号嫌疑人有三类：

| 嫌疑人 | 为什么它会摸这么多对象 |
|---|---|
| **分区表** | 一张 1000 分区的表，一条不带分区裁剪条件的查询就要锁 1000+ 个对象（分区 + 每个分区的索引） |
| **一次性 `pg_dump`** | 或者任何遍历全库的运维脚本 |
| **大量临时表** | 每张都是独立对象 |
...
```
def 申请重量级锁(对象, 模式):
    if 模式 in {AccessShareLock, RowShareLock, RowExclusiveLock} \
       and 对象是本库普通表 and 用的是 DEFAULT lockmethod \
       and 该分区没有强锁存在:                     // 看 FastPathStrongRelationLocks 计数器
        if 本 backend 的 fast path 槽位还没满:      // 上限 16
            记进 PGPROC 私有数组; return           // 完全不碰共享内存
    抢分区 LWLock; 进锁表占一个槽位                  // 第 17 把起、强锁、共享 catalog 都走这里
```

条件那三行来自 `src/backend/storage/lmgr/README`：用 DEFAULT lockmethod；锁的是**本库的普通表**（不能是共享 catalog）；是**弱锁**（`AccessShareLock` / `RowShareLock` / `RowExclusiveLock`）。三个条件同时满足才能走快车道。
...
**那强锁怎么办？** 如果有人要加强锁（`ShareLock` 及以上），它必须能看到那些藏在各个 backend 私有数组里的弱锁。

Postgres 的办法是一个叫 `FastPathStrongRelationLocks` 的计数器数组（1024 个计数器，按分区归类）：谁要加强锁，先把对应计数器 +1；此后其他 backend 发现计数器非零，就知道"这个分区有强锁存在，我不能走快车道了"，老实进锁表。
...
**为什么需要这么多档？** 因为如果只有"读锁/写锁"两档，`VACUUM` 就没法和 `UPDATE` 并存（它俩其实可以），`CREATE INDEX CONCURRENTLY` 也不可能存在。

**每多一档，就是把一类"其实不冲突"的操作从队列里解放出来。** 第三节讲的 `FOR KEY SHARE` 是同一个故事在行级别的重演。
...
商业数据库怎么解决？**锁升级（lock escalation）**：SQL Server 发现某个事务在一张表上的行锁超过阈值（约 5000 把）时，自动把它们合并成一把表锁。

省了内存，但代价极其难受——**你的事务突然就把整张表锁住了**，并发瞬间归零，而且这件事发生得毫无征兆、难以复现。
...
所以"加一把行锁"这个动作，展开来只有两行：

```
def 锁住一行(tuple, 我的事务号):
    tuple.t_xmax      = 我的事务号
    tuple.t_infomask |= HEAP_XMAX_LOCK_ONLY     // 声明"只是锁，没删改"
    // 没有任何共享内存分配；但页面被改脏了，要写 WAL
```
...
**MultiXact 不是免费的**，它是 Postgres 少数几个"能把生产库拖垮"的隐藏成本，账有三笔：

| 成本 | 具体表现 |
|---|---|
| 要写磁盘 | `pg_multixact/offsets` 和 `pg_multixact/members` 两个 SLRU，高频产生 MultiXact 的负载会看到明显的 SLRU 争用 |
| members 空间会被用尽 | 报 `ERROR: multixact "members" limit exceeded` |
| 需要被冻结 | 和事务号回卷一样有 wraparound 风险，靠 `autovacuum_multixact_freeze_max_age` 兜底 |

第二条那个报错来自 `src/backend/access/transam/multixact.c:1260`，原文长这样：
...
def 想锁一行(tuple):
    while True:
        持有者 = tuple.t_xmax
        if 持有者 已结束:  写自己的 xmax; return       // 行到手
        取 tuple 锁（号码牌）                          // 拿不到就在这排队
        等 持有者 的 transactionid 上的 ShareLock       // 持有者提交/回滚时自动被唤醒
        放掉号码牌                                    // 下一位顺位前移
```

一句话：**号码牌决定谁有资格去等，`transactionid` 决定等到什么时候。**
...
📌 **这解释了一个高频困惑**：为什么 `pg_stat_activity` 显示一堆会话在 `Lock:transactionid` 上等待，但你在 `pg_locks` 里找不到它们锁了哪一行？

**因为那个信息不在数据库的内存里，它在数据页上。** 想知道具体哪一行，只能从 `pg_locks` 里 `locktype='tuple'` 那条记录的 `page`/`tuple` 字段反查，或者直接看阻塞者在跑什么 SQL。
...
Postgres 8.x 时代只有两种行锁强度：共享和排他。插入子表要确认"父行还在"，于是给父行加共享锁（当时叫 `FOR SHARE`）；而更新父表要排他锁——两者冲突。

结果是**每插一条日志，就把那个用户的余额更新挡一下**。热门用户的账号直接锁成一条串行队列。
...
一个请求拿不到锁时会进 `ProcSleep`。规则写成伪代码是这样：

```
def ProcSleep(我, 对象, 模式):
    位置 = 队尾                                  // 默认 FIFO
    if 我已经持有该对象上的锁 and 我持有的锁与队列中某些等待者冲突:
        位置 = 第一个这样的等待者之前             // 例外一：否则我会把自己饿死
    if 我的请求 不冲突于 已授予的锁 and 不冲突于 位置之前的所有等待请求:
        直接授予; return                         // 例外二：不用等

def ProcLockWakeup(对象):                        // 有人释放锁时
    for 等待者 in 队列 从队头开始:
        if 等待者 不冲突于 已授予的锁 \
           and 等待者 不冲突于 前面那些还唤不醒的等待者:      // ← (b)，队头阻塞的根源
            唤醒它
```

规则的文字表述来自 `src/backend/storage/lmgr/README`：默认排到队尾（FIFO）；例外一是"进程已持有该对象上的锁、且持有的锁与队列中某些等待者冲突，就插到第一个这样的等待者前面"；例外二是"请求既不与已授予的锁冲突，也不与它插入位置之前的任何等待请求冲突，直接授予"。

唤醒时从队头开始扫，条件是"(a) 不与已授予的锁冲突，且 (b) 不与前面那些还唤不醒的等待者冲突"——**(b) 这一条就是队头阻塞的根源**。
...
| 边的类型 | 什么情况 | 能改吗 |
|---|---|---|
| **硬边**（hard edge） | A 等的锁已经被 B **实际持有** | 不能，这条依赖改不掉 |
| **软边**（soft edge） | A、B 都在排队，A 排在 B 后面且两者请求冲突 | 能，A 等 B 只是因为队列顺序 |

于是 Postgres 会做一件很聪明的事：

```
def DeadLockCheck(等待图):
    环 = 找环(等待图)
    if 没有环:                return 继续正常等待
    for 排列 in 只调整软边的所有队列重排:
        if 该排列不产生新环:  应用它; return 谁都不用死
    选一个事务当受害者 → ERROR: deadlock detected
```

**发现环之后，先试试能不能通过重排等待队列把环拆掉**（只能动软边）。找得到一个不产生新环的排列，就重排，**谁都不用死**。实在拆不掉，才选一个事务干掉。
...
**❌ 不统一加锁顺序会怎样**：死锁在测试环境几乎测不出来（需要两条路径在毫秒级窗口内同时执行），线上流量一大就开始零星出现。

因为频率低，容易被当成"偶发抖动"忽略，直到某天流量翻倍——按并发控制领域的经典估算，**死锁率大致随并发数的平方增长**〔Gray 等人的分析结论，本篇未实测验证〕，于是一夜之间从"偶尔一条"变成"满屏报错"。
...
| 陷阱 | 后果 |
|---|---|
| 连接池复用连接 | 锁被"传染"给下一个用这条连接的人 |
| `hashtext()` 碰撞 | 两个不相干的任务互相排斥 |
| 主备切换 | 锁全部消失，它不是分布式锁 |
| 占锁表槽位 | 细粒度滥用会撞 `out of shared memory` |
...
回到 2.5 节的机制，默认行为是"排队等 `transactionid`"，这两个修饰词各自把它换成别的：

| 写法 | 抢不到锁时 |
|---|---|
| 默认 | 排队等 `transactionid` |
| `NOWAIT` | 不排队，直接报错 `55P03` |
| `SKIP LOCKED` | 不排队，换一行试 |
