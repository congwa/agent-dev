...
> "用 Postgres 当消息队列"曾经是个笑话，现在是主流做法之一。

[river](https://github.com/riverqueue/river)（5.2k★，Go）、[graphile-worker](https://github.com/graphile/worker)、Rails 的 Solid Queue、pgmq，这几个东西建立在同一个特性上：**`FOR UPDATE SKIP LOCKED`**。
...
而用 Postgres 做队列，入队就是一次普通的行写入，可以跟业务写在同一个事务里：
...
先看实测。`labs/exp5b.py`，8 个 worker 抢 64 个任务，每个任务在事务里处理 50ms：

| 方案 | 墙钟耗时 | 任务被领取次数合计 | 重复执行 |
|---|---|---|---|
| **A** `FOR UPDATE SKIP LOCKED`——抢不到就跳过下一行 | **0.44s** | 64 | 0 次 |
| **B** `FOR UPDATE`——抢不到就排队等 | 3.31s | 64 | 0 次 |
| **C** 不加行锁 | 3.48s | 225 | 161 次 ❌ |

表里一共只有 64 个任务，所以"被领取次数合计"超过 64 的那一行就是出事了。

三种写法的失败方式完全不同。

**C（不加锁）**：8 个 worker 同时 `SELECT ... WHERE state='ready' LIMIT 1`，全都拿到同一行，全都执行。**64 个任务被执行了 225 次。** 如果任务是"发短信"，用户收到 4 条重复；如果是"扣款"，那是事故。

**B（普通 FOR UPDATE）**：结果是正确的，但 8 个 worker 全在排队等同一行的锁。**并发度实际上是 1**，8 个进程的吞吐等于 1 个进程，耗时 3.31s。
...
⚠️ 这里最容易读错的是 `LIMIT` 和 `SKIP LOCKED` 的交互。它不是"取 5 行，其中被锁的丢掉"，而是：

```
拿到的行 = []
按 ORDER BY 的顺序逐行往下扫:
    if 这一行被别人锁着:   跳过，继续扫下一行
    else:                  锁住它，放进结果
    if 拿到的行数 == LIMIT: 停止扫描
```

Postgres 会**先按顺序扫描、跳过被锁的行、直到凑够 LIMIT 行**。所以 8 个 worker 各要 5 行时，它们会各自拿到不重叠的 5 行。这正是我们要的。
...
river 的 `JobGetAvailable`（`riverdriver/riverpgxv5/internal/dbsqlc/river_job.sql`）：
...
sub2api 的 outbox 消费（`auth_cache_invalidation_outbox_repo.go`）：
...
**两个独立项目、不同语言、不同场景，写出了几乎一样的 SQL。** 这个模式已经是事实标准了。
...
⚠️ 但要清楚，**`SKIP LOCKED` 会破坏严格的顺序保证。** 如果 job 1 被 worker A 锁着，worker B 会去拿 job 2 并可能先完成它。**队列里没有"严格按顺序执行"这回事**，除非你只用一个 worker。
...
所以 worker 的正确形状是"短事务领 → 事务外干 → 短事务收"：

```
loop:
    jobs = 短事务 { SKIP LOCKED 选中 + 打上 claimed_at/claimed_by 标记 }   // ① COMMIT，行锁立即释放
    for job in jobs:
        result = 处理(job)         // ② 事务外，慢慢来：调 HTTP、发邮件、跑计算
        短事务 {                   // ③
            成功 → 标记完成
            失败 → 记录失败 + 安排重试
        }
...
**❌ 租约设得太短会怎样**：任务还在正常处理，租约就过期了，另一个 worker 把它捡起来重跑 → **重复执行**。
...
原因很直白：部分索引里**只有 500 个索引项**，那 100 万条已完成的行根本不进索引。

还有一个更妙的性质：当一个任务从 `available` 变成 `running` 时，**它会自动从索引里消失**——索引始终只有队列的实际长度那么大。
...
上面那个 115ms 只是 100 万行时的数字。真实场景是这样滚雪球的：
...
**注意那个 `created_at < NOW() - INTERVAL '10 seconds'`——第 6 篇讲过，那是防序列号空洞的。**
...
| **队列会满** | 默认 8GB（`max_notify_queue_pages`），满了会报错 |
